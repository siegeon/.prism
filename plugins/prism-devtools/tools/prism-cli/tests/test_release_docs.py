...
AC-4: plugin.json version is "3.0.0"
...
class TestAC4_PluginJsonVersion:
    ...
    def test_ac4_plugin_json_version_is_300(self):
        """
        AC-4: plugin.json 'version' field is '3.0.0'
        Requirement: Version bump from 2.6.4 → 3.0.0 signals the Brain+Conductor major release
        Expected: json.loads(plugin.json)['version'] == '3.0.0'
        """
        data = json.loads(PLUGIN_JSON.read_text(encoding="utf-8"))
        assert data.get("version") == "3.0.0", (
            f"plugin.json version is '{data.get('version')}', expected '3.0.0'"
        )
