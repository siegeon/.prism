...
AC-4: plugin.json version is "3.0.5"
...
class TestAC4_PluginJsonVersion:
    ...
    def test_ac4_plugin_json_version_is_304(self):
        """
        AC-4: plugin.json 'version' field is '3.0.5'
        Requirement: Version bump from 3.0.3 → 3.0.5 for Brain deps auto-install + messaging (prism-c49b)
        Expected: json.loads(plugin.json)['version'] == '3.0.5'
        """
        data = json.loads(PLUGIN_JSON.read_text(encoding="utf-8"))
        assert data.get("version") == "3.0.5", (
            f"plugin.json version is '{data.get('version')}', expected '3.0.5'"
        )
