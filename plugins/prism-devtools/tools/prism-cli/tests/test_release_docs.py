...
AC-4: plugin.json version is "2.6.1"
...
class TestAC4_PluginJsonVersion:
    ...
    def test_ac4_plugin_json_version_is_260(self):
        """
        AC-4: plugin.json 'version' field is '2.6.1'
        Requirement: Version bump from 2.6.0 → 2.6.1 signals the patch release
        Expected: json.loads(plugin.json)['version'] == '2.6.1'
        """
        data = json.loads(PLUGIN_JSON.read_text(encoding="utf-8"))
        assert data.get("version") == "2.6.1", (
            f"plugin.json version is '{data.get('version')}', expected '2.6.1'"
        )
