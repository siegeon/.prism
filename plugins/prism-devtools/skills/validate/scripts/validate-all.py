...
Usage:
    python validate-all.py                  # Scan prism-devtools
...
def find_prism_root() -> Path:
    """Find the prism-devtools root directory.

    This script lives at skills/validate/scripts/validate-all.py,
    so the root is 3 levels up.
    """
    return Path(__file__).resolve().parent.parent.parent


def find_script(prism_dir: Path, name: str) -> Path:
    """Find a script by name within the plugin directory."""
    candidates = [
        prism_dir / 'scripts' / name,
        prism_dir / 'skills' / 'validate' / 'scripts' / name,
    ]
    for c in candidates:
        if c.exists():
            return c
...
def main():
    prism_dir = find_prism_root()
    scan_root = prism_dir  # Default: scan the plugin itself
    ...
    scripts = {
        'validate-docs.py': find_script(prism_dir, 'validate-docs.py'),
        'validate-refs.py': find_script(prism_dir, 'validate-refs.py'),
        'check-portability.py': find_script(prism_dir, 'check-portability.py'),
    }
    ...
    # scan directory. Passing an absolute --root triggers a path resolution
    # bug producing ~234 false-positive cross-reference errors.
    ...
    if blocked:
        ...
        print(f"  python {scripts['validate-docs.py'].relative_to(prism_dir)}")
        print(f"  python {scripts['validate-refs.py'].relative_to(prism_dir)}")
        print(f"  python {scripts['check-portability.py'].relative_to(prism_dir)}")
        sys.exit(1)
    else:
