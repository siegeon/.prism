...
from prism_service.services import ontology_rules, task_workspace
...
_PROMOTED_MODEL_HEADER = (
    ...
    "# the o:derivedFrom and o:verifiedBy properties every promoted rule\n"
    "# and term carries. services/law_promotion.py writes to this file,\n"
    "# once an owner approves the draft at the promote_to_law workflow's\n"
    "# review gate.\n"
    "\n"
    "@prefix o: <urn:prism:onto:> .\n"
    ...
    "\n"
    "o:verifiedBy a rdf:Property ;\n"
    '    rdfs:label "verifiedBy" ;\n'
    '    rdfs:comment "The test file (and function) that proves this rule '
    'fires on a real violation and stays quiet on a real compliant case." .\n'
)
...
# ---------------------------------------------------------------------------
# A durable, committed regression test for the fixture proof (owner
# 2026-08-27: "the ontology should work with the code and the rules to
# ensure that rules are covered by unit/int tests... tied back to the
# code, and that is where the ontology overlaps"). _install_rule() runs
# this only after its own violating/compliant checks both pass -- see
# below -- so a generated test is never written for a rule that could
# not prove itself.
# ---------------------------------------------------------------------------
...
_VERIFICATION_TEST_TEMPLATE = '''"""Regression test generated by law_promotion._install_rule() the
moment the promoted rule {name!r} first proved itself: it fired on its
...
VIOLATING_FIXTURE = {violating!r}

COMPLIANT_FIXTURE = {compliant!r}
...
def test_{slug}_fires_on_violating_and_stays_quiet_on_compliant():
    from prism_service.config import project_data_dir
...
def _append_verified_by_triple(path: Path, name: str, test_ref: str) -> None:
    """Append `o:<name> o:verifiedBy "<test_ref>" .` onto `path` (the
    same promoted-shapes.ttl _install_rule() just wrote the rule block
    into) -- a plain string literal (a file path + test id, never a
    resource reference), matching how sh:message/rdfs:comment literals
    are already emitted in this file's TTL-writing code."""
    escaped = test_ref.replace("\\", "\\\\").replace('"', '\\"')
    existing = path.read_text(encoding="utf-8")
    path.write_text(
        existing.rstrip() + f"\no:{name} o:verifiedBy \"{escaped}\" .\n",
        encoding="utf-8",
    )
