...
PRISM_VERSION = "7.13.292"

# Changelog-ish notes (free-form; keep short)
PRISM_VERSION_NOTES = (
    '7.13.292: write_failing_tests joins _PLANNED_STEPS, runs its declared narrow middle (task d5808cd1). task_runner was sending write_failing_tests the full blanket defaults (30 turns, $2.00, 900s base = 1800s with 2x multiplier) even though write-failing-tests-loop.json declares haiku / 4 turns / $0.50 / 120s. The full context envelope (~20k workspace CLAUDE.md plus tool schemas) caused context window exceeded errors at 133k tokens. Using the declared budget (haiku, 4 turns, $0.50) limits the context to what the narrow reason-loop prompt was sized for: a DRAFT of a failing test, not the full implementation. The actual implementation (writing tests to disk, running them) lives in the conductor flow after the draft is reported; task_runner just drafts and reports. This pattern mirrors verify_plan, which already joins _PLANNED_STEPS (commit c993779c, task a9f2bec7). The two remaining BUILD steps (implement_tasks, verify_green_state) stay out deliberately: their declared prompts only DRAFT and dispatching them would make drives fast and green on nothing, worse than a timeout. A behavior joins this set when its declaration is real, never by default. Live measure on task d5808cd1 (27 dispatches, 1800s timeouts, 133k token overflows): with the declared budget applied, the step should complete in a single attempt without exceeding the context window.'
