...
PRISM_VERSION = "5.3.23"

# Changelog-ish notes (free-form; keep short)
PRISM_VERSION_NOTES = (
    "v5.3.23: `prism status` no longer lies. Customer report on Linux "
    "native install: status said 'stopped' against a running daemon on "
    ":7777/:7778. Root cause: cmd_status only checked the .pid file, "
    "but anything starting the daemon WITHOUT going through `prism "
    "start --daemon` (Tauri shell spawn, `python -m prism_service.main`, "
    "docker container, systemd, customer's manual launch) doesn't "
    "write a pidfile. Fix: also probe http://127.0.0.1:<ui>/api/version "
    "via urllib. Status now reports (a) running with pidfile when "
    "started via the CLI's daemon mode, (b) running without pidfile "
    "when something else spawned it (with explanation), (c) stopped "
    "only when both pidfile is absent AND the port doesn't respond. "
    "Also shows the served version when it differs from the CLI's "
    "version — catches the 'two prism-service installs in the same "
    "env' confusion. "
    "v5.3.22: Tauri shell is properly cross-platform now. Removed three "
    "Windows-specific hardcodes that broke Linux / installer builds: "
    "(1) the service-dir cwd no longer defaults to the v6-pivot worktree "
    "path — `find_service_dir()` walks up from current_exe() looking for "
    "pyproject.toml + prism_service/ next to it, so it Just Works whether "
    "the binary is at target/debug/prism-shell (dev), inside an installed "
    ".msi (next to a bundled `services/prism-service/` dir), or anywhere "
    "else; (2) python invocation goes through `find_python()` which picks "
    "the Windows 8.3 short path on win32 if it exists (dodges argv-split), "
    "`python3` on POSIX (PEP 394 canonical); overridable via PRISM_PYTHON; "
    "(3) the spawn no longer forces PRISM_DATA_DIR — the python "
    "resolve_data_dir() does the right thing per OS "
    "(%LOCALAPPDATA%\\prism on Windows, ~/.prism on Linux/macOS, /data in "
    "docker). Linux .AppImage builds + macOS .dmg builds now spawn the "
    "service correctly without depending on a developer's host paths. "
