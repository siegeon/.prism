...
PRISM_VERSION = "7.12.36"

# Changelog-ish notes (free-form; keep short)
PRISM_VERSION_NOTES = (
    "7.12.36: 7.12.35's solid-green pill for a done conductor task was itself dishonest -- green_gate passing is a claim about the SDLC (all gates cleared against the task's own isolated worktree), never a claim the code reached anyone. Owner caught this directly looking at the live app: the button 7.12.35 showed green for was still visibly on screen. Cross-referenced the rail against GET /api/tasks/stranded (the real, already-existing, git-backed signal for whether a [task:<id8>] trailer actually resolves on origin/main) -- a done task now renders solid green ONLY if genuinely shipped, amber (validation's own 'Terminated' tone) if stranded. Investigating why /stranded returned 0 rows (it should have flagged 4f74dafc) surfaced a SEPARATE, bigger, live-data bug: understand_state.json's source_path for this project was still 'E:\\\\.prism' (the pre-AOS-migration Windows path), so _project_source_path resolved to a repo this Linux instance can't read, origin/main never resolved, and /stranded silently early-returned empty -- for EVERY feature depending on that path, not just this one. Corrected the live config to /home/siegeon/projects/prism (the real checkout this whole session has been committing to); /stranded immediately went from 0 to 202 real stranded rows, 4f74dafc among them exactly as expected. "
