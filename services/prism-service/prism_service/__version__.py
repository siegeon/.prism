...
PRISM_VERSION_NOTES = (
    "v6.8.1: SDLC LIFECYCLE is now a first-class, MCP-TAUGHT concept + all six "
    "phase-skills exist and are version-controlled. The six phases (discover -> "
    "prototype -> implement -> verify -> ship -> operate) were aspirational: only "
    "implement was state-machine-enforced, only 2 skills were tracked (.claude/ "
    "was gitignored, so ship/prism-operate/sync-main lived on-disk-only and never "
    "reached main), and discover/verify didn't exist. NOW: (1) models/workflow.py "
    "gains a LIFECYCLE model (Phase = id/title/purpose/DELIVERABLE/TIER/skill/ "
    "STATUS/steps) beside WORKFLOW_STEPS — ONE source of truth for the arc, with "
    "phase_for_step + lifecycle_doctrine() + lifecycle_registry(); each phase "
    "carries a model-agnostic capability tier (composes with the v6.7.33 role "
    "engine: discover/prototype=frontier, implement=fast, verify/ship=balanced, "
    "operate=fast) and an HONEST enforcement status (enforced|partial|skill_only). "
    "(2) The getting-started MCP surface TEACHES it to any connecting model: "
    "mcp/instructions.py Server(instructions=) now leads with the lifecycle arc "
    "then role doctrine; prism_guide gains a 'lifecycle' section (right after "
    "overview) rendering the arc + a phase->skill->deliverable->status table; the "
    "orchestration section + prism_onboard point at it. No new module — extends "
    "instructions.py + tools.py. (3) .gitignore un-ignores ONLY .claude/skills/ + "
    ".claude/workflows/ (machine-local settings/launch/worktrees stay ignored), so "
    "ship/prism-operate/sync-main/byos-test are now tracked. (4) NEW real skills "
    ".claude/skills/discover (requirements -> Artifact, frontier) + verify (quality "
    "matrix: tests/storybook/a11y/docs/static, balanced) fill the two gaps. "
    "Tests: prism_guide/onboard/profiles/two-surface (35 green). "
