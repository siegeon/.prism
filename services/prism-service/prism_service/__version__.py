...
PRISM_VERSION = "7.13.376"

# Changelog-ish notes (free-form; keep short)
PRISM_VERSION_NOTES = (
    '7.13.376: the plan step is explicit, not prose. THE FROZEN ROUND (task e40c3efb, 2026-09-14, code held at 7.13.375, no hand after the flip): review, story and story_gate in two minutes by machine, then three plans in 24 minutes, each refused by plan_gate for the same missing words, RED at base, while the colour step had handed the planner exactly those words every time. Owner: we keep leaving this up to the model, be very explicit about what is supposed to happen, the explicit comes from the planning steps. THE WORDS ARE NO LONGER THE MODEL TO CHOOSE. verify-plan-loop.json goes version 8 to 9: recall, colour, compose, loop, text-challenge. (1) compose (POST /steps/plan-compose, block plan.compose) is zero-model: PRISM states the pinned test and its colour at base, writes AC-1 itself (red_at_base on the pinned test), lists the files in scope and the task oracle, and defines the terms from the lexicon. (2) loop returns TYPED slots only: goal, acs as objects with colour an enum of exactly red_at_base and guard, oracle, pytest_id, files_to_change; no markdown, no diagram. (3) the plan_structured rubric (block plan.render) renders plan_doc and plan_diagram in the exact shape the gate teeth parse, inserts the pinned red AC when the model omitted it, then runs form_complete, plan_diagram_parses and plan_coverage AT THE STEP, so a bad draft is refused where the retry is cheap and its reason is what plan-refusal-recall hands the next attempt. (4) the runner ends a refused verify_plan attempt with that reason and never falls back to the inline prose prompt. (5) ontology/model-lexicon.ttl declares Oracle, RedAtBase and RegressionGuard with the alt labels people write, so the enum, the teeth and the vocabulary are one thing. 9 tests. NOT YET OBSERVED LIVE: the next frozen round is the measurement. '
    '7.13.375: the colour step names a pinned test that does not exist yet. Live on 6bc3e6c2 one minute after 7.13.374: plan-base-colour answered rc=None, NOT MEASURED, because the pinned file test_every_agent_node_runs_its_declared_steps.py does not exist at base 23f90490 (nor on main), so pytest could not collect it. That is the COMMON shape for new work and it is red by construction: the task creates the file. The step now checks each pinned path at base with git cat-file -e before running anything, reports colour=absent, never spends a pytest run on a file that is not there, and hands the planner the words the tooth accepts: RED at base: <pytest id> (the file is absent before the fix), every other AC stays green. 1 new test, the measured-red test now pins the file as present. '
    '7.13.374: the planner is told the pinned suite colour at base, and the already-shipped closer measures on the released tree. TWO LIVE FINDINGS under 7.13.373. (1) 83dcd479 still did not close: the closer now recognised its shipped-direct tag, but measured task.verify at the plan BASE, where a hand-shipped ticket is red because its workspace baseline predates its own ship. That red is the evidence the work exists, not evidence it is missing; a65c66e5 had only passed because its baseline postdated the landing. already_shipped now runs the pinned suite on origin/main, where shipped is true, and the finding says so. (2) 6bc3e6c2 rewound correctly on the coverage refusal, drafted ACs with oracle lines, and was then refused by already_green_ac with say which criterion is RED at the base commit, and measure it there, addressed to a tool-less narrow planner that cannot run anything; rewind 2 of 3 spent on an impossible instruction. verify-plan-loop.json goes version 7 to 8: a codified colour step (POST /steps/plan-base-colour, block plan.base_colour) runs task.verify at base with the tooth own _run_at_rev once and exports ${baseColourBlock}: RED hands the planner the exact words the tooth accepts (RED at base: <pytest id>) and tells it every other AC is a guard; GREEN says the work is already done or ONE NEW red test must be named; unmeasured asks for an explicit colour. The loop prompt interpolates it after ${refusalBlock}; _build_step_variables carries an empty default so the no-chain fallback never ships the placeholder. 7 new tests plus the routes pin updated for the fourth step. '
    '7.13.373: round two at plan_gate, two parks fixed in the seat that parked them. (1) Task 83dcd479 was hand-landed as a chore(version) commit 572c3f75 with tag shipped-direct-572c3f75 and NO [task: trailer, so the 7.13.372 closer found nothing and the plan kept refusing (every AC a guard, budget spent, parked for a person). already_shipped now also accepts a shipped-direct-<sha> tag whose commit rev-parses and is an ancestor of origin/main, then requires the same green pinned suite at base; the finding says which of the two proved it. (2) Task 6bc3e6c2 flip-flopped every sweep between plan_coverage: story carries no AC-<n> ids and plan_gate: the rubric gave no actionable refusal, because plan_rewind._refusal_for read only the deterministic teeth at plan_gate and answered empty for a RUBRIC refusal; story_gate had read its rubric all along. plan_gate now falls back to the rubric verdict, so the refusal reaches verify_plan and the recall step hands it to the planner. A zero-AC refusal was first tried in form_complete and REVERTED: fixtures for other teeth are plans without AC entries and eleven tests broke; the defect was the reader, not the tooth. 3 new tests; 197 green across the plan-gate, adjudicator and rewind suites. Still red on main and not mine: test_a_refused_rubric_rewinds.py::test_the_module_imports_no_policy_module (plan_rewind imports oracle_spec since 3336a17d). '
    '7.13.372: an already-shipped ticket is closed at plan_gate by the machine seat, never planned again. THE LIVE ROUND (task a65c66e5, 2026-09-14): the ticket carried tag shipped-direct-f547a89 and commit f547a892 on origin/main carries its [task: trailer; its pinned suite was green at base, so NO acceptance criterion could ever be red. Every tooth was right: two form rewinds (no oracle lines), one red-at-base rewind (every AC a guard), the whole rewind budget, an hour of the single engine slot, then a park for a person with no acceptance criterion is shown to FAIL at the base commit. Nothing asked the one question that decides it: is this work already on main. FIX AS A NODE STEP AND A CLOSER. plan_gate_checks gains CLOSERS = (already_shipped,), a positive finding (never a refusal, CHECKS is unchanged) when the squash-safe trailer reader api/tasks._shipped_sha_on_main finds the task on origin/main AND task.verify runs green at the plan base via the same _run_at_rev the red-at-base tooth uses; no pins, no trailer, or an unmeasurable run answers empty and the round continues. run_check reports it as ok=True with close=True, GateCheckStatus carries close, and plan-gate-check.json (version 5 to 6) runs already_shipped as its FIRST step so the canvas counts it. gate_adjudicator.close_if_already_shipped runs before any plan refusal in the sweep: one write to green_gate/passed/done with the finding on gate_reason, a gate_decide history row naming conductor-adjudicator, and the ordinary rubric path untouched when it finds nothing. 8 tests. NOT YET OBSERVED LIVE: the next adjudicator sweep of a65c66e5 is the first real pass. '
    '7.13.371: an interactive MCP result never exceeds what the engine carries inline. THE LIVE DEFECT, read from the run stream of task bb3d1f6a implement_tasks (2026-09-14, dispatch 10 and 11 of 12): the opus agent called brain_understand(query=prism_service, limit=20, depth=1), PRISM returned 118,077 characters, the Claude Code engine refused to carry it (exceeds maximum allowed tokens), wrote it to a tool-results file under its private HOME and told the model to Read it back; the model mistyped that path, hunted for the memory file for 810 seconds and then 272 seconds, exited 1 with an empty think wrapper each time, and the dispatch guard re-ran the identical attempt. The single engine slot was held the whole time, so a65c66e5, rewound to verify_plan at 02:07 and ready for the new recall step, was skipped as engine-busy for an hour. No PRISM tool bounded its own output; the engine ceiling was the only limit and its failure mode is a file path. FIX AT THE TOOL BOUNDARY: mcp.tools.handle_tool now passes every interactive result through _bound_result_size. Over the cap (PRISM_MCP_RESULT_MAX_CHARS, default 60,000 characters, about 15k tokens, under the engine default of 25k with headroom for the reflection banner; <= 0 disables) the result becomes ONE json.loads()-able envelope carrying truncated, tool, chars, limit, a hint naming the lever (lower limit/depth, one entity not a package) and the head of the result, shrunk until the ENVELOPE fits because JSON escaping grows the head. Automation-profile callers are exempt, they parse raw JSON. 4 tests; 71 green across the handle_tool suites. '
    '7.13.370: the runner activity row names the drive, not a sweep. Owner 2026-09-14: it is a REACTIVE system, what sweeping is there really. Read in source: task_runner._loop has no clock; it blocks on a wake event (a task change or a freed engine slot, 7.13.364), and the pass that follows lists eligible tasks and then BLOCKS inside run_one_step for the whole inference call. The running row read sweep_once for all of it, so a 480 second drive of bb3d1f6a implement_tasks looked like a clock tick that did nothing while a65c66e5 queued behind the single engine slot with no visible reason. FIX: the pass_ label is now woke: checking eligible tasks, and sweep_once takes the live pass entry and renames it to driving <task8> <step> the moment it picks, which the cross-process exporter republishes every 0.5 s. A step lookup failure degrades to the task id alone; a pass that picks nothing leaves the label untouched and still collapses as idle. 4 tests. Same posture owed to ship_worker, gate_adjudicator and resume_actuator, whose rows still say sweep_once; not changed here. '
    '7.13.369: test_bodies shape is owned by the schema, not by haiku escaping JSON inside JSON. Observed live on task bb3d1f6a, three passes in a row after 7.13.365 (02:07, 02:15, 02:25 UTC): reason-loop returned exit 0 with 268 to 686 output tokens, final_text was only the empty think wrapper, and the structured_output carried test_bodies as a JSON-ENCODED STRING that failed json.loads (Expecting comma delimiter at char 306, an unescaped docstring quote), with name copied from the pinned pytest id (path::test_name) and a body that restated the fixed def line. _assemble_test_draft refused each with a bare not a JSON list, refusal-recall handed that back, and the next attempt made the same three mistakes. THE COST WAS SYSTEM-WIDE: the engine serves one request at a time, so task a65c66e5, rewound to verify_plan at 02:07 and ready to pass through the new recall step, was skipped as engine-busy on every pass behind that loop for 25 minutes. FIX IN THE NODE. write-failing-tests-loop.json goes version 13 to 14: the loop schema declares test_bodies as an ARRAY of {name, body} objects so the constrained decoder owns the shape and the model never escapes JSON by hand; red-prompt-compose now says bare function name, no path, no ::, statements only, no def line, and do not JSON-encode the list inside a string. The assembler normalises what still arrives wrong: path::name resolves to the bare pinned name, a leading def name(): line is dropped and the body dedented, a {name: body} dict is accepted as the same information, and an invalid encoded string is refused WITH the parse error and the first 90 characters of what arrived, so the next recall names the exact defect. 7 new tests in test_test_bodies_shape_is_owned_by_the_schema.py including the live shape as a fixture; 71 green with the drafting and red-node suites. Reported, not fixed here: tests/unit/test_run_pinned_suite_declares_expected_rc.py has 6 red on main since bbea5b26 (its fakes emit no FAILED line, so the named-target tooth from e1c0e373 refuses them), a peer landing fixture defect. '
    '7.13.368: the two drafting sub-nodes become registered blocks, so the canvas counts them. Owner rule 2026-09-13: every seat behaviour is a registered pydantic Block referenced by id from the flows, counted on the canvas, and a Python-only fix does not count. refusal-recall and test-scaffold shipped in 7.13.362 as declared node steps with real routes, which made them visible as STEPS but not as BLOCKS, so they sat outside the registry the canvas reads. Both are now Block entries in blocks/red_blocks.py beside the targets, context-pack, prompt-compose and materialize blocks: red.refusal_recall, deterministic, reading the newest reason-loop row and surfacing a test_drafted refusal only while it is the newest, so a later pass self-clears it; and red.test_scaffold, deterministic, computing the pinned file and def lines from task.verify plus verified imports and real signatures parsed with ast, asking the code graph first and falling back to the file on disk because the index cannot be trusted warm. Seven red blocks now register, 17 in total. No behaviour changed: each block delegates to the SAME route the node file already declares, so this is a registration, not a second implementation. 22 green across the two registry suites. '
    '7.13.367: the assembler accepts the shape a small model actually emits, and says what it got. 7.13.365 moved the drafting model off emitting a whole file and onto per-test bodies, and the very next two live drives of task bb3d1f6a refused with test_bodies is not a JSON list of name and body entries. THE FLAW WAS MINE, in the contract not the model: the declared schema types test_bodies as a STRING holding JSON, so a compliant answer is JSON nested inside JSON, and a small model emits the natural container instead. A name to body MAPPING carries exactly the same information as a list of name and body pairs, so the assembler now normalises a dict into that list and proceeds. It stays STRICT in what it assembles: every pinned name must still be present, an unpinned name still refuses by name, an empty body is still a missing one, and the assembled file still goes through the unmodified score_test_drafted rubric. Only the container shape is liberal. Second, the refusal now names the shape it received and shows the first 120 characters of it. The old text said only that the value was not a list, which tells the model nothing to change, and the refusal-recall node hands that exact text to the next attempt as its one piece of feedback: a reason that does not say what arrived wastes the retry it feeds. One existing test that pinned a dict as a refusal is updated IN PLACE with a comment naming what superseded it; it still refuses, now because the key is not a pinned name rather than because the container is a dict. 2 new tests, 14 green in the assembly suite. '
    '7.13.366: the plan_gate refusal reaches the planner, so a rewind is not a blind retry. Observed live on task a65c66e5 after 7.13.364 landed: the new form tooth rewound plan_gate to verify_plan (Rewind 2/3, plan_checks: 3 of 3 AC(s) carry no oracle line), and verify-plan-loop.json then re-ran the IDENTICAL static prompt, which never asked for an oracle line and never carried the refusal. Attempt 3 would have produced the same shape and escalated a form defect to the owner after all, exactly what the tooth was built to stop. THE FIX IS IN THE NODE. verify-plan-loop.json goes version 6 to 7: a codified recall step (POST /api/workflows/steps/plan-refusal-recall, zero model calls) now runs before the loop step and exports refusalBlock, the framed plan_gate refusal read from task.gate_reason, which plan_rewind writes on every rewind; the loop prompt opens with ${refusalBlock} and gains rule 9, the required per-AC shape with an indented - oracle: sub-bullet. Self-clearing by construction: a plan that passes never rewinds, so gate_reason stops carrying a plan refusal and the block is empty. Registered as the pydantic block plan.refusal_recall in blocks/plan_blocks.py so the canvas counts it. _build_step_variables also carries refusalBlock from the same pure function, so the no-chain fallback prompt never ships the literal placeholder. 8 new tests in test_plan_refusal_reaches_the_planner.py; the routes pin in test_verify_plan_dispatches_its_declared_steps.py updated in place for the third step. 254 green across the node, block and verify_plan suites plus the two integration suites that touch verify_plan. NOT YET OBSERVED LIVE: the next drive of a65c66e5 is the first real pass through the recall step. '
    '7.13.365: the drafting model stops emitting a file, and a coloured FAILED line stops breaking the red gate. TWO PARTS, both measured on live drives of task bb3d1f6a. (1) OUTPUT SHAPE. The loop step asked a small model for a whole Python test file inside a JSON string, plus the file path. Consecutive real drives produced: imports unresolvable module(s) prism; test_code does not parse (unterminated string literal at line 8); draft does not define pinned test id(s) test_a_semicolon_and_a_contraction_still_go; test_code does not parse (unexpected character after line continuation); test_file_path is empty. Every one is an OUTPUT-shape failure, and each costs a full call on an engine that serves one request at a time. The path was never the model business: the codified test-scaffold step already derives it from task.verify, along with both required def lines, verified imports and real signatures read from disk with ast. So the schema now asks for test_bodies only, a JSON-encoded list of name and body pairs, plus expected_failure_reason. A new codified _assemble_test_draft builds the real file from the scaffold authoritative parts and those bodies, wired into _score_rubric BEFORE score_test_drafted so assembly never bypasses a single existing check: it still must parse, define every pinned id, hold a real assert, resolve its imports and raise nothing before an assert. A body for an unpinned name, a missing body for a pinned name, and an empty body all REFUSE by name rather than inventing an empty function. A fields dict with no test_bodies key passes through untouched, so every other caller is unchanged. test_file_path is empty is now structurally impossible. Node file version 13, all 11 steps intact. (2) A LIVE RED-GATE BREAKER, found because a test of mine went red and I read it instead of filing it as pre-existing. The new named-target tooth requires a FAILED line naming a pinned target, which is a genuine honesty improvement over a bare rc==1. But this environment pytest emits ANSI even under capture_output, so the real line reads FAILED then an escape reset then the path. The literal match for FAILED plus target therefore NEVER matched, and run-pinned-suite refused EVERY honestly red suite with no pinned target id appears in a FAILED line, which makes red_gate unsatisfiable for every task. Reproduced directly: a bare-file target and an exact file::test id both refused at rc=1 with a genuine failure present. The runner now passes --color=no, and _not_red_reason strips ANSI anyway, because a check that matches a literal must never depend on a flag staying present when the failure mode is silent. The stored tail is now readable evidence instead of escape-littered text. 12 assembly tests plus 2 ANSI regression tests including a negative control proving an unrelated failure is still refused; 122 green across the drafting and red-step suites. '
    '7.13.364: freeing the engine slot now wakes the queue, so a task deferred for a busy engine no longer waits on an event that cannot come. Observed live on task bb3d1f6a. The adjudicator rewound it to write_failing_tests, a non-gate step, so the drive worker could take it. The worker swept, found the single engine slot busy with another task, and skipped it. That other drive finished, the engine sat idle, and bb3d1f6a was never attempted: its history showed rewind with no runner_attempt after it. THE CHAIN, each link read in source. task_runner._loop ends every pass on _wake_event.wait(timeout=_fallback_timeout_s(...)). 7.13.357 deliberately made that fallback opt-in only, and PRISM_WORKER_FALLBACK_S is unset on this daemon, so the timeout is None and the worker blocks. task_runner.wake() has exactly one caller, task_service._publish_task_changed, and wakeups.signal(task_changed) likewise. dispatch_guard.end_dispatch popped the ticket and stopped its heartbeat thread while signalling NOTHING. So the slot becoming free reached nobody, and ordinary queueing fell to resume_actuator, whose 180 second sweep exists for CRASHED drives and carries a per-task retry ceiling: using it as the work queue burns a budget meant for failures and mislabels waiting as a stall. THE FIX KEEPS THE REACTIVE POSTURE the owner asked for; it does not restore a blind periodic timer and it does not set an env var. (1) THE SIGNAL: end_dispatch calls task_runner.wake() right after popping the ticket, lazily imported and best effort. It takes no branch on why it was called, so it fires on every exit path, success and exception alike, by construction rather than by inspection. Both real callers were confirmed to end a dispatch in both their success and their except paths, so one fix point covers all four. (2) THE BELT AND BRACES: eligible_tasks records when it skipped a candidate for a busy engine slot, sweep_once clears that flag at the top of every tick so a quiet tick can never inherit a stale one, and a new pure _wait_timeout_s NARROWS the wait to 5 seconds only when that flag is set. It never lengthens the wait, an explicit shorter PRISM_WORKER_FALLBACK_S still wins, and _fallback_timeout_s itself is untouched so its pinned contract still holds. CONFIRMED AND PINNED BY TEST: a sweep that finds nothing eligible still blocks indefinitely. 10 new tests, 7 red first; 147 green across the dispatch-guard, concurrency, resume-actuator and idle-fallback suites. Three integration failures in test_task_runner_headless_drive.py were bisected rather than assumed pre-existing: on a clean checkout the file failed three different ways across three runs, which is the documented _drive_now background-dispatch race, not this change. '
    '7.13.363: the reindex button can finally fix coverage, because it now walks the source. Task 4b15f4bc, measurement in memory mx-e908cf. Measured against /home/siegeon/.prism/projects/prism/brain.db: 1186 docs total, only 8 distinct .py files present out of 249 in the service, services/lexicon.py and services/arc_governance.py both at zero. THE CAUSE was not staleness. POST /api/brain/reindex called Brain.incremental_reindex, whose candidate set is git diff HEAD plus git ls-files --others and nothing else, with no baseline walk anywhere. So the button an owner presses could never index a file that was committed and clean, no matter how many times anyone pressed it. I saw this live and could not explain it: last_reindex stamped one minute before my check while find_symbol still returned nothing for align, load_lexicon and score_test_drafted. The diff-only scope is DELIBERATE for the recurring drift sweep and two tests pin it, so it stays. The full walker source_service.ingest_source_to_brain already existed, and only bootstrap_after_clone ever called it, at project-configure time, never again. The 8 covered files were indexed inside one six-second window on 2026-09-11/12, purely from being dirty during a drift sweep. FIX: /api/brain/reindex now calls the full walker. POLLUTION, two parts. (1) .playwright-mcp joins source_service._INGEST_SKIP_DIRS, which Brain._EXCLUDED_PATH_SEGMENTS aliases, so _should_index, is_ingest_excluded and the walker all honour it at once. Those browser page dumps contributed more than 83 docs and ranked inside the top three hits for a code query, because the brain_index_doc MCP tool checks is_ingest_excluded and applies NO suffix restriction, so any agent pointing it at a page .yml sailed through. (2) A blank-content guard now sits at BOTH doc write entry points, BrainService.index_doc and Brain._index_files; neither had one, and five docs held no content at all. REFUSED, on evidence, and recorded so nobody reverses it by accident: excluding __version__.py. Task f9e0745e deliberately caps only the oversized module chunk at 4 KB and lets the sliding-window tier keep covering the file so nothing is lost, and the live numbers match that design exactly, one module doc at 4092 bytes plus 459 windows of 2048 chars with 256 overlap across 878 KB. Its 39 percent share of the index is a COVERAGE artifact: index 249 files instead of 8 and the share collapses on its own. A new test pins the keep-it behaviour, and the honest lever for the window count, if anyone wants it later, is a wider stride for changelog-class files, never exclusion. 10 new tests against synthetic repositories, never the live database, 5 of 10 red first; about 280 tests green across every suite touching is_ingest_excluded, ingest_source_to_brain, index_doc and brain_index_doc. '
    '7.13.362: the test-drafting node stops guessing what it can compute. Owner standing order: make the workflow nodes more programmatic so PRISM uses as little inference as possible. The measured waste here was never the declared model call, it was the RETRIES a wrong draft causes, and write_failing_tests averages 27,398 tokens per pass. write-failing-tests-loop.json goes version 6 to 8 and now reads route, gather, recall, scaffold, loop, write, run, commit: SEVEN codified steps around ONE inference call, up from five. TWO NEW CODIFIED SUB-NODES. (1) refusal-recall (task 08e666ff) returns the most recent test_drafted refusal for this task and only when the newest reason-loop row is both not ok and carries a reason starting test_drafted, so a later pass self-clears the block with no delete path anywhere. It reads the existing agent_runs_data.get_agent_runs helper rather than a new sqlite connection, isolates by task, and degrades to an empty block on any error. Before this the refusal text was computed, recorded and never read back, so the next attempt re-ran an identical static prompt and could return the identical bad draft. (2) test-scaffold computes the pinned file path and the exact required def lines from task.verify, reusing the existing _pinned_test_names_for_file parser, then resolves the real symbols. It asks brain_svc.find_symbol first and falls back to reading the file from disk and parsing it with ast, because THE INDEX CANNOT BE TRUSTED TO BE WARM: measured against /home/siegeon/.prism/projects/prism/brain.db, the prism code graph holds 8 of 249 service Python files, __version__.py alone supplies 460 of 1186 docs (39 percent, it is an 878 KB changelog chunked into windows), and services/lexicon.py has zero docs. That is the real reason task bb3d1f6a drafted from prism_service.lexicon, a module that does not exist, and called load_lexicon with an argument when the real function takes none. Verified live for bb3d1f6a with the index still cold: the block now carries both pinned test names, from prism_service.services.lexicon import align, and the true signatures def align(text: str) -> tuple[str, list[dict]] and def load_lexicon() -> list[Term]. Every emitted import passes _submodule_path_resolvable first, so the node can never print a path it has not resolved, and anything it cannot find on disk either degrades to a plain could-not-confirm line instead of a fabricated signature. The disk search is bounded to basenames the task text actually names, never a repo walk. Also fixed: a dotted span like ontology/model-lexicon.ttl parsed as module.symbol and produced the bogus candidates ttl and tsx, and the unresolved list is capped at 5 names plus a count in the block while the response field still reports all of them. 39 new tests across two files, each red first; test_test_drafted_refusal_stops_the_chain.py updated IN PLACE for the two new steps. 111 green across the eight neighbouring node suites. Index coverage itself is NOT fixed here and is tracked as task 4b15f4bc with the measurement in memory mx-e908cf. '
    '7.13.361: red_gate stops dead-ending onto a human, because the rewind now reads the RECEIPT instead of a stale string. Task bb3d1f6a sat at red_gate with live readiness saying the machine seat already swept this anchor and will not retry, and this gate needs a distinct actor decision now. The owner rule is that red_gate belongs to the machine seat and a machine abstention at red is a system defect, never a click to request. The backward edge already existed: plan_rewind.maybe_rewind (task 1bcb2b24, 2026-09-08) rewinds red_gate to write_failing_tests. It only recognised ONE of the two non-red shapes, because _refusal_for matched task.gate_reason.lstrip().startswith(NOT red), which fires only when the pinned suite genuinely PASSES at the anchor. bb3d1f6a hit the OTHER shape, a collection error at rc=4 whose reason text starts with red not demonstrated at, so the match never fired and maybe_rewind returned inconclusive forever. This is the repo own recurring sin, recorded twice already in lessons: deciding from the stored gate_reason snapshot rather than from live evidence. THREE PARTS. (1) oracle_spec.red_refusal_kind(receipt) returns passed, collection_error or inconclusive, read from the pytest_pass observation rc, because run_red_oracle funnels BOTH non-red causes into status ST_FAILED and the status alone cannot tell them apart. (2) plan_rewind._refusal_for reads the real latest receipt through that classifier. Both passed and collection_error now rewind to write_failing_tests; inconclusive still never rewinds, mirroring green_rewind own distinction, so a runner that could not judge at all never triggers a redraft. It reuses the existing rewind_budget and rewind_count machinery scoped to red_gate, so nothing parallel was invented and the cap still parks loudly. (3) api/conductor.py readiness is now cause-specific and honest. It previously asserted the pinned suite passes there for ANY non-red receipt, which was a false statement about evidence on an rc=4 where pytest collected nothing, and it always closed with needs a distinct actor decision. A rewindable cause now reports that the machine seat is redrafting on its next sweep with no owner action needed; a spent budget or a seat that is off says so plainly; only a genuinely inconclusive receipt still asks for a person. Anchor movement verified by reading _red_step_sha and _red_tests_commit: tried keys on (red_sha, spec_hash), a new tests-only [task:<id>] commit moves the anchor, and the cb1dc6f4 self-heal prefers the freshly resolved commit over a stale recorded red_step_sha row. 18 tests across three files, red first; two existing suites updated IN PLACE with supersession comments, including one whose premise that no machine path exists stopped being true when plan_rewind shipped. '
    '7.13.360: three computed facts that the test-drafting chain threw away now decide something. Read live off task bb3d1f6a, whose task.verify pins TWO pytest ids in one file: the drafted test defined ONE of them, the rubric passed it, write-test-file wrote it, run-pinned-suite measured rc=4 (pytest cannot collect a test id that was never written) and reported outcome ok anyway, and commit-tests-only committed it as the task red anchor. red_gate demands rc==1, so that anchor can never be demonstrated. Each fix acts on information the code already had. (1) COVERAGE: score_test_drafted learns an optional pinned_ids evidence key and refuses a draft that does not define every pinned test function for its own file, naming the missing name. New pure helpers parse path::Class::name with any bracket parametrisation stripped and match the draft path by suffix in either direction plus a basename fallback, so a workspace-root-relative pin and an absolute task_workspaces pin both match. workflows._pinned_ids_for reads task.verify and returns an empty list on ANY failure, so a missing task or an empty verify requires nothing and every existing caller is unchanged. (2) SUBMODULE IMPORTS: the import check resolved only the top-level name, so from prism_service.lexicon import load_lexicon passed on prism_service alone while the real module is prism_service.services.lexicon, which is the SAME rc=4 collection error one redraft later. It now resolves the full dotted path, but ONLY when every ancestor package is already in sys.modules, because find_spec on a child imports its parents and this checker must never execute code the process has not already loaded. Indeterminate still passes; a checker that rejects good tests is worse than no checker. (3) THE DECLARED rc: the run step of write-failing-tests-loop.json (version 5 to 6) now declares expected_rc 1, and run-pinned-suite refuses when the measured rc differs, carrying rc, tail and paths into the refusal plus a reason naming both integers and what the measured one means. expected_rc None keeps report-only behaviour for every other caller. The declaration lives in the NODE FILE, not in a Python branch. Enabling fix underneath it: _dispatch_declared_steps computed stopped via getattr(result, stop_chain, False), which is ALWAYS False for a dict-returning handler, so no codified step could ever stop its own chain; a new _step_field reads the field from a pydantic response or a plain dict, and the stopped row now carries the step own reason instead of an empty string. That is the actual mechanism that let commit-tests-only run on rc=4. Verified live in-process against the real committed draft and the real pinned ids: refused naming the missing test id; refused naming prism_service.lexicon once coverage was fixed; ok with 2 pinned ids covered once both were fixed; unchanged when no ids are pinned. 40 new tests, each red first. NOT DONE and reported plainly: no symbol-level import check, so a draft importing a real module but a name that does not exist in it still passes, and that needs real execution to catch. '
    '7.13.359: the test-drafting node gains a GATHER sub-node, so it can ground its imports in the real repository. Owner 2026-09-13 asked for capability added as NODES in the flow rather than more Python guards. Read from the live run log, task bb3d1f6a drafted a test whose FUNCTION NAMES matched task.verify exactly and whose assertions used the right domain terms (Skill, Rationale, Provider, Convention, Party) -- the model understood the job -- but it opened with from prism.ontology import Term and from prism.service import create_ticket. There is no top-level prism package (it is prism_service), so pytest died at COLLECTION with rc=4 where red_gate demands rc==1. The node runs narrow (no tools, empty scratch dir) and simply could not see what modules exist. THREE PARTS. (1) Enabling change: _exported_variables (task_runner.py) exported only from a plain dict result or a result.reason dict, so a TYPED pydantic step response exported nothing at all and could never feed the next step. It now also reads model_dump() (falling back to dict()/vars()), keeping the existing scalar-only filter and the snake_case plus camelCase double export. That unlocks every typed step feeding the chain, not just this one. (2) New sub-node: write-failing-tests-loop.json goes version 4 to 5 with a gather step calling the EXISTING /api/workflows/steps/context-enrich route before loop, at timeoutSeconds 30 since it is a local ContextBuilder lookup and not a model call; its StepEnrichResponse.brain_context now exports as ${brainContext}. Registering context-enrich in _step_handlers was required too -- a declared route with no handler is reported, not silently skipped. (3) The loop prompt hands the model that material and tells it to import ONLY modules that appear in it, keeping the ${verify} and ${oracle} material 7.13.353 added. The chain is now oracle-route-check, context-enrich, reason-loop, write-test-file, run-pinned-suite, commit-tests-only, and _runs_as_declared_steps still returns True because the build trio is undisturbed. 3 new tests red first then green; two exact-call-sequence assertions in test_test_drafted_refusal_stops_the_chain.py updated IN PLACE to include gather, with a comment naming what changed -- they stay exact and still prove a refusal blocks the build trio. 271 tests green across every consumer. NOT FIXED and reported plainly: the same draft was also TRUNCATED mid-string at about 1700 chars, and no max_tokens or output-length flag exists anywhere in claude_cli.py or config.py -- the cap is enforced by the external local-inference server, outside this repo. '
    '7.13.358: /api/conductor/state stops recomputing every task because one task moved. Measured in a real browser: 400-505ms and 130-144KB on EVERY call, on the hot path for Dashboard, Sidebar and Conductor alike, while the same endpoint served warm in 5-7ms. The N+1s were already gone; what remained was cache-INVALIDATION granularity. _phase_progress_cache_key keyed each task memoized progress on _db_change_stamp() -- PRAGMA data_version, a WHOLE-FILE write counter -- chosen as a superset to notice a CHILD row moving, since children_done/children_total depends on children and the parent own updated_at cannot see them. It was wrong in BOTH directions. LIVE it over-fired: data_version moves when ANOTHER connection commits, and the worker host is a separate process writing heartbeats and history rows continuously, so one busy task evicted all ~59 managed tasks cached progress on nearly every request, each miss re-reading and re-parsing live Claude transcript JSONL (81,819 json.loads across 502 files cold). IN-PROCESS it under-fired: data_version stays put for our OWN connection commits, so a child written through the same service never invalidated its parent at all -- a correctness bug, not just a cost one, pinned by the new test_phase_progress_cache_is_task_scoped.py. The stamp is now derived from self._task_svc.list(parent_id=task_id), the SAME parent_id-scoped read _phase_progress_uncached and _children() already issue, which TaskService memoizes per parent_id per db-write-generation (_parent_list_cache) -- so whichever call site runs first pays the query and the rest are dict hits, at ZERO additional SQL. That last point was the hard constraint: a first attempt added a COUNT/MAX aggregate per task and broke test_conductor_state_parent_scoped_reads_are_not_reissued_per_task, which caps parent-scoped statements at one per task per render; that test was verified to PASS on clean code, so the regression was real and the attempt was reverted rather than landed. Instrumented both ways: 40/40 parent-scoped statements before AND after. 3 new tests red first then green, query-cost suite 8/8, 178 green across every phase_progress consumer. The one failure, test_conductor_child_lookups_use_parent_id_scoped_list[phase_progress], reproduces on a CLEAN baseline -- it regex-scans phase_progress own body for a literal that lives in _phase_progress_uncached, a separate function, on HEAD already. '
    '7.13.357: the last fixed-interval worker goes reactive, and a refused drafted-test rubric actually stops the chain. Owner 2026-09-13: this is a REACTIVE system, not a passive constant scanning system looking endlessly for rework; even while driving a task it should be quiescent. (1) A worker audit found EIGHT of nine standing workers already on the reactive contract from task b490fabc -- gate_adjudicator, language_alignment, resume_actuator, ship_worker, deploy_worker, dispatch_guard and maintenance_clock all wait on a real signal with worker_fallback_s() returning None unless an operator opts in, and all take their wait baseline AFTER the sweep so a worker own write can never re-trigger it. task_runner._loop was the one never migrated: it waited on its private _wake_event with timeout=PRISM_TASK_RUNNER_INTERVAL, so on this instance it ran a full sweep_once -- spend ceiling, system load, engine reachability, get_all_projects, per-project eligible_tasks -- every 900 seconds with zero task activity, unconditionally. New _fallback_timeout_s defers to the shared opt-in-only contract in production while keeping the plain interval for the test-only stop_event path. wake() already made it reactive for real events; only the fallback was still on a wall clock. NO cross-worker feedback loop was found: task_service.update() is the sole task_changed emitter and the self-retrigger class was already closed by the post-sweep baselines. (2) workflow_step_reason_loop computed a rubric verdict and never told the caller about a refusal. _dispatch_declared_steps already had a generic early-exit on result.stop_chain, but ReasonLoopResponse never set it, so a refused test_drafted verdict still let write-test-file, run-pinned-suite and commit-tests-only run -- which is how task bb3d1f6a committed a test importing from prism.ontology, a module that does not exist (find_spec(prism) is None while prism_service resolves), leaving pytest to die at collection with rc=4 where red_gate demands rc==1. ReasonLoopResponse now carries stop_chain, set only when a rubric was declared AND its verdict is not ok, and the recorded row carries the refusal text instead of an unconditional ok:true so the retry path can report it. A passing verdict or a node declaring no rubric behaves exactly as before. 8 new tests across two files, each red first; 737 unit tests green across every neighbouring suite. Two integration failures in test_task_runner_headless_drive.py were checked rather than assumed: test_wake_cuts_the_wait_short reproduces on a CLEAN baseline (pre-existing), and test_spend_unbounded_when_ceiling_env_unset passes in isolation -- the headless-drive claim collision with the live daemon this file is already documented for. '
    '7.13.356: the gate sweep stops re-examining finished tasks, and the Tasks board stops refetching identity on every task event. Owner, 2026-09-13: the application is unresponsive again, this should feel instantaneous. Measured on the live instance rather than reasoned about. SERVER: gate_sweep_rows selected WHERE workflow_step IN (...) with NO status predicate, so a task that closed long ago -- nothing clears its gate workflow_step on close -- came back on every pass forever. Of 336 rows the sweep fetched, 327 were finished and only 9 were live work; 272 were done + green_gate + passed. Adding a FINISHED_STATUSES exclusion (done, cancelled, deleted, archived) cuts the fetch 336 -> 9, a 97.3 percent reduction, measured against the real task DB. The predicate excludes by STATUS and never by STEP, deliberately: gate_adjudicator own terminal-step-closure branch (workflow_step == done -> _close_if_terminal) still needs to see a LIVE task sitting at a terminal step. This churn is also what emitted task_changed continuously, which is why language_alignment_worker fired every ~5.5s around the clock and the single-slot local engine (--parallel 1) stayed busy for 14 hours at 91 percent CPU. CLIENT: a 60s IDLE window on /tasks with zero clicks issued 85 requests and about 6.6 MB -- 20x /api/tasks at 333KB, plus 20x /api/auth/me and 20x /api/workspaces, because TasksPage bundled identity and the external-workspace merge into the SAME task_changed-gated load() as the board itself, though neither changes when a task changes. They now run once per mount/project change behind their own effect with a cancelled guard, removing about 40 of those 85 idle requests. RULED OUT by measurement, having previously been asserted: stale chunks, the in-place restart, and the project selector appear NOWHERE in the network trace; the static shell is fast (TTFB 3ms, DOMContentLoaded 17ms) and ordinary API calls are 2-34ms. A 4.8s interaction reading was reproduced four more times at 50-100ms and is recorded here as a measurement artifact of automated-browser timer throttling, NOT an app defect -- do not cite it. STILL OPEN and next: /api/conductor/state costs 400-505ms and 130-144KB on EVERY call and sits on the hot path for Dashboard, Sidebar and Conductor alike; and the workers remain poll-shaped rather than reactive, which is the architectural fix the owner is asking for. '
    '7.13.355: commit-tests-only can finally commit a test, which is why no task in PRISM had ever reached a red anchor. Measured across the live scores.db agent_runs table: commit-tests-only ok=0 twenty-five times and ok=1 ZERO times, while every other declared node succeeded (write-test-file 25/25, run-pinned-suite 24/25, reason-loop 91/91, oracle-route-check 20/20). All 25 failures carried the identical reason: not tests-only, 1 non-test path(s) are dirty (tests/). THE BUG: git status --porcelain COLLAPSES a brand-new untracked directory into one entry with a trailing slash, so the guard saw the literal string tests/, took f.rsplit(/,1)[-1] which is the EMPTY STRING, found it did not start with test_, classified its own test directory as a stray non-test path and refused to commit. A new test file always lands in a directory git is not yet tracking, so this could never have worked. Fix in api/workflows.py workflow_step_commit_tests_only: list untracked files individually with --untracked-files=all, strip rename arrows keeping the destination path, and move the stray test into a _is_test_path helper that rstrips a trailing slash so an empty basename can never be misclassified either way. The guard KEEPS its purpose: a genuinely dirty non-test file still refuses the commit. 2 new tests in test_commit_tests_only_recognises_nested_test_files.py drive a REAL git repo in tmp with no mocking of git: a test file in a never-tracked directory now commits for real (git show --stat confirms it), and a dirty non-test file alongside a test still refuses with git log showing zero commits. Proved red first and re-proved red independently by reverting the patch, where the failure output shows files: [tests/] -- the collapsed directory itself. 79 tests green across every suite referencing this node. Note the pre-existing test_commit_tests_only_records_its_own_run_on_a_direct_call only ever exercised a ROOT-LEVEL file, which is why it never caught this. This is the last link: 7.13.351 bounded the step, 352 made a park releasable, 353 handed the node its material and 354 stopped an advance on prose -- all real, but all guarding a pipeline whose final node discarded the work regardless. '
    '7.13.354: write_failing_tests can no longer advance on prose, which is what permanently destroyed a task ability to ever demonstrate red. Live evidence, task bb3d1f6a: an attempt returned an empty think wrapper followed by chat (I have examined the test files and found...), no test file was written, and the runner stamped red_step_sha 23f904900062 and advanced to red_gate anyway; red_gate then refused forever with rc=4 could not collect, because the pinned suite passes at that anchor so red can never be produced from that history. ROOT CAUSE: _run_one_step decided success purely on proof being non-empty plus a graceful exit, and the existing _DRAFT_ONLY_WITHOUT_CHAIN guard only stops a draft-only prompt being sent ALONE -- once it falls through to the full-brief fallback (BUILD_TOOLS, no declared chain) a model with tools can simply TALK about the tests and return exit 0 with non-empty prose, which read as pass. Fix: a new _write_failing_tests_has_artifact treats a proof as real only when it carries a def test_ definition OR a pytest node id; the OR is required because test_task_runner_red_ids_on_live_path own realistic fixture is pytest tail output with no def test_ in it, and an AND broke that pinned test. A no-artifact attempt now takes the EXISTING failure path -- advanced=false, retry, and eventually _handle_stall parks it with a real reason -- rather than any new stall mechanism. 2 tests, proved red first against the literal proof string from bb3d1f6a history, then re-proved red independently by reverting the patch; 258 neighbouring tests green. MEASUREMENT THAT CORRECTS AN EARLIER READING: this task declared-chain routes (reason-loop, write-test-file, run-pinned-suite, commit-tests-only) appear NOWHERE in its history, so the declared chain never dispatched for it at all and every attempt ran on the full-brief fallback WITH tools. The node was therefore not blind here; 7.13.353 handing a declared node its material is correct but applies to a path this task never took. Why the declared chain does not dispatch is the next open question. '
    '7.13.353: a declared node is handed its material, so the agentic middle stops being asked to hit a target it is never shown. THE FACTORY DEFECT. Every agentic node in the conductor ran haiku with narrow=True (no tools, empty scratch dir, so it cannot read the repository) and _dispatch_declared_steps was called with exactly three variables: taskHint, taskId, project -- and _build_task_hint returns only title plus description. All five *-loop.json nodes interpolate only taskHint. Measured live on task bb3d1f6a: write_failing_tests was told Draft a failing test for this task: <title+description> and was NEVER told the pytest ids it had to produce, while red_gate demands exactly task.verify (test_creating_a_ticket_swaps_no_words.py::test_prose_keeps_the_words_the_author_wrote and ::test_a_semicolon_and_a_contraction_still_go). All four attempts returned an empty think wrapper, the step advanced with no artifact anyway, and red_gate refused with rc=4 could not collect -- an anchor from which red can never be demonstrated. That one starved dispatch explains the fabricated tool-call markup in a plan_doc, the placeholder AC-123/AC-456/AC-789 plan, and the empty test drafts alike: not five defects, one blind factory. Fix: new _build_step_variables returns taskHint, taskId and project byte-identical to before, ADDING verify (the pinned pytest ids, newline joined), oracle, allowedFiles, stopIf, planDoc and title; every missing field renders the empty string, never the word None. write-failing-tests-loop.json (version 3 to 4) now names the exact pinned target and the oracle in its prompt. ALSO FIXED before landing, caught in review: _declared_agentic_prompt fell back to a bare replace(${taskHint}) for verify_plan and write_failing_tests, so on the one path taken precisely when the declared chain cannot dispatch, the model would have been handed the literal text ${verify}; it now runs the same full _subst. 4 tests, all proved red first (the fallback guard failing on the literal placeholder surviving), green after; 98 neighbouring tests green. Necessary companions, not replacements, for 7.13.351 (an unbounded step) and 7.13.352 (an unreleasable park): a task still could not complete while the node it reached was blind. '
    '7.13.352: a step-retry park can be released, so the board can drain. Measured live: 19 blocked tasks, only 5 releasable, 14 with NO release path at all. POST /api/conductor/park/release recognised exactly two prefixes, dispatch-guard: and resume-actuator:, and answered 409 not a governance park for anything else -- but task_runner._park writes a THIRD kind with no prefix (step <name> did not advance after N attempts), which 7 live tasks were sitting in. Confirmed live against task bb3d1f6a. The consequence is worse than it sounds: fixing the CAUSE of a park does not revive the tasks it already killed, so 7.13.351 removing the unbounded-step defect left every task that defect had parked permanently dead. task_runner gains PARK_PREFIX step-retry:, a _LEGACY_PARK_RE that also matches the existing unprefixed wording so the 7 already-parked tasks are covered with no migration, is_step_retry_park(), and its own release() mirroring the two sibling seats. _park now prefixes new reasons. park/release recognises the third kind and calls all THREE seats release() rather than only the matching one -- honouring the b490fabc lesson recorded in that route own docstring, where releasing one seat left a sibling counter at its ceiling and the task re-parked on the next sweep, so the owner had to click Release twice. No separate counter reset was needed: _stall_count already treats any history row matching status blocked -> in_progress as a fresh-mandate boundary (_OPERATOR_RESET_RE, task_runner.py:254), verified in source rather than assumed, so the status flip zeroes the count on its own. 4 new tests in test_step_retry_park_release.py, one asserting the BEHAVIOUR (stall count reads 3 before release and 0 after, on a task whose history still carries all three attempt rows) rather than a status string. Proved red first, then re-proved red independently by reverting the patch and re-running. 2 assertions in test_park_release_route.py updated in place: the route contract legitimately grew from 2 seats to 3. 355 neighbouring tests green. KNOWN GAP, flagged not fixed: dispatch_guard.GOVERNANCE_PARK_PREFIXES does not include step-retry:, so conductor_flow/mcp flow_start could still resurrect such a park before a release -- the same latent bug the other two seats already guard against, left as its own iteration. '
    '7.13.351: a declared step timeout now actually binds, so a conductor step can no longer run unbounded. Three live tasks were blocked on a step killed past its budget (bb3d1f6a and b2f29d45 are fresh repros; aa9e7842 turned out to be a stale pre-fix snapshot from 2026-09-09 whose reason quotes advice the current code names as the wrong first move). The 7.13.287 over-grant theory was checked and RULED OUT: the allowed_tools empty-tuple fix and the narrow-context scratch dir are both already ancestors of origin/main. The real hole is one layer deeper. .prism/behaviors/conductor/write-failing-tests-loop.json declares timeoutSeconds per step (30/120/30/660/120) and _node_plan already parses it into steps[i][timeout_s] (task_runner.py:339), but _dispatch_declared_steps read only route and body and called the handler with no bound at all, and workflow_step_reason_loop called claude_cli.invoke with no timeout_s. invoke defaults timeout_s to None and does run_kwargs = {} when it is None, so subprocess.run received NO timeout and the TimeoutExpired branch was unreachable on that path -- the agentic middle of the very fix meant to make these steps cheap could run forever, until dispatch_guard reaped it externally. Fix: _dispatch_declared_steps merges a declared, positive timeout_s into the body when the body does not already set one, and ReasonLoopRequest gains an optional timeout_s that is passed to invoke. Merged into the body rather than added as a third positional arg so every existing 2-arg handler keeps working. Default None preserves todays behaviour for any node declaring no timeout. The loop step of write-failing-tests now binds its declared 120s. SIDE EFFECT, deliberate and flagged: the run step RunPinnedSuiteRequest has its own timeout_s default of 600, and its declared 660 now reaches it, tightening an unused declaration into an honored one; no test pinned the old default through the declared-dispatch path. 2 new tests in test_declared_step_timeout_binds.py assert the BEHAVIOUR (the declared value reaches the handler body, and reaches claude_cli.invoke), proved red first and re-proved red independently by reverting the patch. 174 neighbouring tests green. STILL OPEN: TIMEOUT_EXIT_CODE is -9, the same value as a real SIGKILL, so _failure_reason cannot tell PRISMs own timeout from an external reap and claims the former unconditionally -- a false attribution worth its own ticket. '
    '7.13.350: a task can no longer enter blocked without saying why. Measured on the live board: 19 of 22 live tasks were blocked and 6 of them carried an EMPTY blocked_reason (b9772333, 8848089d, d7947eb6, dc815149, 6f18c224, d0b392b3) -- a red row naming nothing any driver, machine or human, could act on. Every WORKER path already passed a reason (dispatch_guard.py:359, resume_actuator.py:230 and :480, ship_worker.py:1036, task_runner.py:1830); the hole was the generic status PATCH, which had transition guards for in_progress (the session gate) and done (the open-gate close guard) but none for blocked, and the SPA button sent {status} alone. api/tasks.py now refuses the TRANSITION into blocked with no blocked_reason, using the same shape as its two neighbours and a new BLOCKED_NEEDS_REASON_FIX message; a row already blocked is grandfathered, so editing another field never demands the reason be retyped, and leaving blocked is untouched. Refused rather than auto-filled on purpose: an invented reason would satisfy the letter and destroy the point. TaskDetailPage setStatus now asks for the reason before sending, and a cancel or an empty answer aborts instead of posting a wordless block. 6 new tests in test_blocking_a_task_states_a_reason.py, proved red first (the wordless PATCH answered 200), including two source-reading UI pins that strip comments before asserting, because this change own comment names blocked_reason and would otherwise satisfy them with no code behind it. 923 neighbouring tests green. NOTE: tests/unit/test_workflows_section_ui.py fails 2 tests identically with this change fully reverted -- pre-existing red on main, WorkflowsPage.tsx is byte-identical to origin/main here. The 6 existing reasonless rows are left alone: inventing reasons for them would be the same sin this guard refuses. '
    '7.13.349: the root plan_gate certainty score now reads the PLAN, not the ticket. Measured on the live prism DB: every gate_decide row carrying a certainty value was an approval at exactly 1.00 -- 21 of 21 -- so the conductor-adjudicator seat had never once refused a root plan_gate on certainty. Proof case, task bb3d1f6a: a review seat rejected its plan_gate at 18:03:33 because plan_doc was fabricated tool-call markup, the daemon re-planned in about 90 seconds, and the seat approved the replacement at 18:05:04 with plan_completeness 1.0, oracle_quality 1.0, diagram_quality 1.0, scope_alignment 1.0. That replacement carried invented AC ids (AC-123, AC-456, AC-789) and ZERO oracle lines. ROOT CAUSE: all four signals grade SHAPE. plan_completeness is a word count against a floor, so padding clears it. oracle_quality reads task.oracle and task.likely_misfire, the TICKET AUTHOR fields, never the plan own per-AC oracle lines. diagram_quality only needs the mermaid to parse with more than one edge, so a generic box diagram passes. scope_alignment compares the plan CLAIMED files against allowed_files, so a plan citing nothing contradicts nothing and scores 1.0. The emptier the plan, the less there is to fail. FIX: a fifth signal, plan_grounding, reads the plan body and scores the fraction of AC-<n> ids that carry an oracle line. It is a VETO MULTIPLIER on the four-signal base, never a fifth averaged member: averaging it in RAISED a packet that the original four had correctly parked (the rich-plan vague-oracle fixture went from 0.875 parked to 0.9 cleared), a regression the existing suite caught. A path-existence check was built and then DROPPED on evidence: scored over the 391 live plans it flagged 110 as citing only non-existent files, and reading the samples showed almost all were false positives, because plans cite paths relative to the PACKAGE root (api/tasks.py, web/src/pages/SettingsPage.tsx, services/lexicon.py) rather than the repo root. BLAST RADIUS measured before landing: of 391 plans carrying a plan_doc, 150 carry an oracle line under every AC, 151 carry none, 5 are partial, and only 13 of the zero-oracle plans are still live. 4 new tests in test_plan_gate_certainty_reads_the_plan.py assert against the REAL approved artifact, checked in as a fixture: 7.13.187 already shipped per-signal isolation tests for this exact one-constant-wearing-four-names failure, and they pass while live data stays pinned at 1.00, because they assert over synthetic packets. Proved red first, the real placeholder scoring 1.0 against the 0.9 threshold. Two exactly-four-signals assertions in test_adjudicator_minds_the_root_plan_gate.py are retired in place with a note naming what superseded them; the invariant they protect, that no signal is a constant wearing several names, is kept across five. 234 tests green across every suite reading design_packet or certainty. '
    '7.13.348: opening Settings no longer issues one request per project. Measured live on the AOS dev instance with a real browser: GET /settings fired 154 sequential GET /api/understand?project=<name> calls, one for every project the instance has ever seen (gac-*, cwl-*, beatrank-*, plus throwaway fixture projects such as doesnotexist12345 and nonexistent-zz), because SettingsPage mounted an effect calling loadAllInfos(projects) regardless of which section was on screen. The resulting infos map is read ONLY by the projects section cards, so every other section paid the whole fan-out for nothing -- including Settings > Access key, the page a user must open to switch Remote assist on. That is why the owner reported Settings as not responding, and why the daemon read as unresponsive while such a tab was open. Fix: section is resolved before the effects, and the loadAllInfos effect returns early unless section is projects, with section added to its dependency array so arriving at that tab still loads the cards. 3 new source-reading tests in test_settings_project_fanout_ui.py (the SPA has no JS test runner, per the established convention): the guard exists, the dependency array re-runs on a section change, and infos is read only inside the projects section so a future reader cannot silently reintroduce the stampede. Proved red first against the pre-fix source (2 failed, the premise test correctly green), 3 green after. tsc --noEmit rc=0 and npm run build rc=0. '
    '7.13.342: four standing-worker passes stop paying full price when nothing changed. Team lead tick-cost brief, 2026-09-13, measured live at 7.13.338 with zero in_progress tasks and the inference engine stopped: gate_adjudicator.sweep_once 136.8s then 94s+ across two back-to-back passes over 26 unchanged pending gates, deploy_sweep 77s then 46s per tick, language_alignment 6s per tick, every route hung while they ran. (1) gate_adjudicator.sweep_once used to call TaskService.list(), a project-wide SELECT * plus a full row-to-task conversion, once per project per pass regardless of how few tasks were gate-relevant; it now reads a new lean TaskService.gate_sweep_rows() (id/workflow_step/gate_state/updated_at only, narrowed at the SQL predicate too), and the existing per-task backoff still holds across it, so a second pass over an unchanged set of pending gates adjudicates nothing. The activity feed now reports N at gates, M changed, K decided so the backoff holding is visible rather than inferred. (2) deploy_worker.sweep_new_land ran a real git fetch plus git rev-list on every tick even when nothing had landed; _upstream_ahead_count now caches the ahead-count per repo root for 15 minutes, and deploy_after_land (the ship path) calls a new signal_land() so a real land is never hidden behind the TTL -- a direct push (this repo own self-dev carve-out) is still caught within the TTL by the ordinary poll. (3) language_alignment_workers unforced run_once_for dry-run scanned every tasks free text on every tick regardless of whether the project had moved; a new TaskService.text_fingerprint() (COUNT, MAX(updated_at), one indexed aggregate) is checked first, and an unforced pass over an untouched project now skips before ever calling language_alignment.align_language. (4) drift_reindex per-file content hashing plus per-project backoff (round 6 of the live-page hang, a project that stays continuously in use was swept every ~5s forever because git diff HEAD names an unchanged dirty file as a candidate on every pass): reused unmodified from a sibling fixer branch (fix/livehang) whose two commits were already tested and unlanded -- brain_engine.incremental_reindex now sha256-hashes each candidate against a hash persisted in that Brain own index_meta table and skips a match entirely, and drift_worker backs a zero-embed pass off (60s doubling, capped 15 minutes) while a real embed resets it to immediate eligibility. governance.detect_duplicates was checked and left unchanged: already fully incremental (a persisted per-entry hash cache) and GIL-chunked as of 7.13.337, confirmed still returning fast with no real work when nothing changed. 33 tests green across test_deploy_seat.py, the new test_gate_adjudicator_sweep_cost.py, test_align_language_workflow.py, and the drift suites; the wt-tickcost/wt-condperf in-flight worktrees were read for reference only, never touched. NOT ADDRESSED here: the single-slot engine contention (--parallel 1) that makes even a cheap, correctly-gated pass block every route while it holds the slot -- a concurrency question, separately owned. '
    '7.13.345: a shared change-counter poll layer for the SPA, cutting the request storm an idle Workflows tab was generating (owner 2026-09-13 02:20Z, live measurement: 665+ requests in 7 minutes across ~10 independently-polled endpoints -- version, staleness, workflows/live, conductor/state, consolidation/workers, tasks, tasks/stranded, jobs, sse/work -- because every consumer ran its own fixed-interval fetch with no notion of whether anything had actually changed). New GET /api/changes (api/changes.py) answers a single bumped counter read straight off services/wakeups.py existing signal bus -- the same mutation points that already wake standing workers -- so it mints no new entity or storage. New lib/useChanges.ts polls it ONCE per tab, module-scope singleton (exactly one self-rescheduling timer regardless of subscriber count, the same sharing shape sharedStream.ts already uses for SSE), paused entirely while the tab is hidden and resynced immediately on focus/visibilitychange. Measured live against an OLDER backend that predates this route (404 forever): the naive 1Hz poll cost ~60 extra req/min for zero benefit, so useChanges backs off to a 30s cadence after 5 consecutive failures and snaps back to full speed on the very next success. New generic lib/usePolledResource.ts is the refetch gate any GET-backed query should sit behind instead of its own private setInterval: refetch when the shared counter moves, on window focus, or at a 30s floor, never while hidden, with a module-scope stale-while-revalidate cache per URL so navigating back to an already-fetched resource paints instantly and revalidates in the background. lib/api.ts gains fetchConditional/api.getConditional, an opt-in ETag-aware GET (sends If-None-Match, treats a 304 as a no-op reusing the cached body) so a route that starts sending ETag costs no body on an unchanged refetch, plus a dev-only (import.meta.env.DEV) request-rate console.table every 10s naming which endpoint is actually being hit. Concrete before/after wired end to end: Sidebar.tsx useStaleness moved off its own bare setInterval(tick, 5000) (still hidden-gated, but polling forever even when nothing was ever stale) onto usePolledResource. Backend pinned by 6 new tests in test_api_changes.py (TestClient against the real app: zero-state, per-project signal bump, wildcard-signal bump, cross-project isolation, no-project-sees-everything, tiny two-field payload). Frontend pinned by 14 new source-reading tests in test_polling_discipline_ui.py (no JS test runner, so these assert the actual TSX/TS source per the projects established convention) covering the singleton timer, the backoff, hidden/focus handling on both new hooks, the 30s floor, the module-scope cache, the ETag support, and Sidebars old-interval removal. 179 neighbouring and new tests green (Sidebar/staleness, workflows API, conductor-state coalescing, shared SSE stream). MEASURED LIVE end to end against the running daemon (Playwright, real project=prism data, 946+ tasks): a single-tab, focused, idle /workflows page against the unmodified build averaged 209 req/min; the SAME page served through this changes build (proxied to the same live backend, which still lacks /api/changes so useChanges backs off as designed) averaged 176 req/min, a 16% cut from Sidebars staleness poll alone plus the backoff keeping the new counter poll itself cheap against an old backend. Scope note: this lands the shared mechanism plus one concrete consumer (Sidebar); the other high-volume endpoints named above (tasks, tasks/stranded, jobs, workflows/live, conductor/state, consolidation/workers) live in page components other in-flight fixes own and were left untouched by file-ownership agreement -- they can adopt usePolledResource the same way Sidebar did, and the full win (near-zero idle polling) lands once the backend carrying /api/changes actually deploys and more consumers gate on it. '
    '7.13.334: the daemon was opening ~6 concurrent claude -p dispatches while services/local-inference/launch.sh serves exactly ONE request at a time (--parallel 1) -- owners own screenshot, 2026-09-13 00:20Z: load 15, the daemons own API routes hung. Two independent dispatch paths had no shared concurrency cap at all: task_runner.sweep_once threads up to PRISM_TASK_RUNNER_CONCURRENCY (default 4) tasks at once, and dispatch.pys _drive_now instant handoff fires a brand-new background thread on every single step advance with zero concurrency check of its own. Fix lands at the ONE existing chokepoint every real dispatch from every seat already passes through, services/dispatch_guard.pys try_begin/end_dispatch: a new PRISM_DRIVE_CONCURRENCY env var (default 1, read once and cached) now bounds how many dispatch tickets may be open at the same time, counted from this processs own open tickets (a live step-agent process it spawned) unioned with any live drive_heartbeat row from either autonomous seat -- never from a tasks status column. task_runner.eligible_tasks, dispatch._drive_now, and resume_actuator.dispatch_once each consult the same read-only engine_slot_reason as an advisory pre-check (so a busy slot costs nothing before try_begins atomic reservation), and a busy refusal is a DEFERRAL, never a spent retry-budget attempt. dispatch.after_step/on_started now report drove=false with the reason, recorded once (de-duped) on the tasks own history so the Trace shows queued rather than nothing. conductor_service.activity_for gained a new queued state, distinct from the alarm word stalled, for an in_progress agent-step task whose own lack of motion/heartbeat is explained by another task genuinely holding the one slot. Declared as driveConcurrency on .prism/behaviors/conductor/bot.json so /workflows can show the ceiling. Root cause of an incidental regression this surfaced (test_rest_drive_is_visible_on_the_board.py AC-5/AC-6): dispatch_guard._OPEN_TICKETS is process-global, so a real (if doomed) background dispatch triggered by flow_report/flow_starts own handoff could leak an open ticket across unrelated later tests in the same pytest process -- fixed with a new autouse conftest fixture mirroring the existing adapter/observer registry isolation pattern, plus a second fixture clearing drive_heartbeats between tests (a couple of pre-existing tests dispatch under the fixed project name prism, and a heartbeat row from one task stayed live for an unrelated tasks slot check). 10 new tests in test_engine_drive_concurrency.py, plus the full neighbouring dispatch_guard/task_runner/resume_actuator/drive_heartbeat/activity suites green. NOTE: tests/integration/test_task_runner_headless_drive.pys test_spend_unbounded_when_ceiling_env_unset and test_accumulator_charged_immediately_after_invoke_returns fail identically with this change fully reverted to base (7.13.290s own note already names this file a headless-drive claim collision with the live daemon that reproduces on base) -- confirmed pre-existing, not a regression from this change. '
    '7.13.331: the Workflows canvas never blanks while the backend is merely slow. WorkflowsPage.tsx had no distinct not loaded yet state -- workflows seeded as an empty array, so the directory rail, the canvas, and the status lines No runs yet / No recent activity all rendered their genuinely empty copy whether the board was truly idle or the first GET /api/workflows / GET /api/conductor/state simply had not answered yet, confirmed live taking over 10 seconds during a post restart drift reindex burst (the owner, repeatedly: the click through is a dark solid image again like i asked you fix over and over). Adds a combined dataLoaded flag (catalog arrived AND conductor state observed, from useConductorStates own observed flag) that gates every such empty state render; a canvas loading overlay with a live elapsed seconds counter (Loading workflows... (catalog Ns)); a directory rail skeleton; a backend slow · Ns pill for a slow (over 5s) or failed refresh poll that keeps showing the last good data instead of blanking; and a sessionStorage catalog cache (readCatalogCache/writeCatalogCache) so a fresh mount repaints instantly from the last good catalog while refetching. Pinned by 5 new source-reading tests in test_workflows_canvas_loading_state_ui.py; 427 workflows/conductor-adjacent tests green. Verified live via a real production build served through vite preview behind a delaying proxy: screenshotted both the loading overlay (Loading workflows... (4s), rail skeleton, no false No runs yet) and the loaded canvas with a live backend slow · 14s pill. '
    '7.13.325: plan_gate now catches a plan_diagram that cannot render, not just one that opens with a known keyword. arc_governance.mermaid_parses (the plan_coverage rubric e2 tooth) only checks the first non-empty line, so task a65c66e5 reached plan_gate carrying a diagram whose last node label was never closed (E open-bracket Validate change with gate tests, no closing bracket, no further edges), and it scored ok while PlanView told the owner Diagram failed to render on the exact screen plan_gate exists to let them review. New tooth plan_gate_checks.plan_diagram_parses runs a real structural check, a bracket and quote stack over square, round and curly delimiters plus quote balance, and names the offending line; it degrades to pass on an empty diagram or one whose first line already fails mermaid_parses, so the two teeth never double refuse the same defect with two different messages. Wired into CHECKS and LABELS, so both machine seats that already read plan_gate_checks.run_all, the entry time autoclear in conductor_flow and the gate_adjudicator resweep, withhold on it the same way they withhold on the four existing teeth; no separate per seat plumbing was needed because both already call the shared registry. Registered as its own plan-gate-check.json step (version 5) so the Workflows page shows a real node for it, the same shape as the other four teeth. verify-plan-loop.json (version 6) gains an eighth authoring rule telling the planning model to close every bracket and quote every label that carries punctuation, naming task a65c66e5 as the real failure the rule exists to prevent. 13 new tests in test_plan_diagram_parses.py, pinned against the actual live a65c66e5 diagram fetched from the running task, plus 100 green across the neighbouring plan_gate and arc_governance suites. '
    '7.13.294: the Workflows page shows live activity at every surface, and reads calm when nothing runs (task 0b5dd37c). Before this, only the run-history rail moved, and only while a scripted run was in flight; a person who selected a workflow with no active run saw a static strip and read it as a hang. That was reported live: qa-5aaf1e29 selected Build and test, did not click RUN WORKFLOW, and flagged the still ticker as a possible stall. The page now computes ONE presentation state, LiveTier, as running, settling, quiet or disconnected, and every surface renders from it. The order of the test matters: disconnected is decided FIRST, because we cannot see it and it is broken must never read the same as nothing is happening. RUNNING puts an accent dot on the directory row that is alive, at EVERY nesting depth, both in the top-level catalog map and inside renderBranch, with no depth or tier guard, so a person scanning the collapsed list sees which row is busy without opening anything. The canvas gains drawFlowUnits and FLOW_UNIT_SPEED: small units travel along the token wire into the step that is active now, so what MOVES BETWEEN STEPS is visible, not only which box is lit. A Recent workflow activity ticker carries the last three real events, capped by RECENT_EVENT_LIMIT, from step transitions the page already observes. SETTLING is a ONE-SHOT flourish inside SETTLE_WINDOW_MS of 6000 ms, in the verdict tone, on the last rail pill, the row dot and the terminal canvas node, then everything stops. QUIET IS THE POINT OF THE TICKET, not an afterthought: it never animates, and it never uses an alarm word. The status line says No run in progress, last run, then a relative time and a verdict, or No runs yet. The words idle, stalled, frozen and no active driver are absent by test, because those words tell a person to intervene somewhere and there is nothing for them to do. Motion is driven by real occupancy only, a running run or a task whose activity state is working or driving, never by a timer that always ticks: a page that always moves would make a real stall read as healthy, which is the misfire this ticket named up front. Every animation added uses transform and opacity only, never width, top, left, height or an animated box-shadow, so the compositor can hold 60fps. 22 source-reading tests in test_workflows_page_live_tiers.py pin all of it, and they were PROVEN RED first: run against the pre-change sources, 17 of 22 fail. The 5 that pass are pre-existing invariant guards, a negative word check on both pill tones, existing copy, the keyframe discipline scan and the already-shipped ActiveNodeProgress tone field. Those 5 are regression guards and are NOT evidence that this feature works. '
    '7.13.293: revert of 7.13.292, which put write_failing_tests into _PLANNED_STEPS and pushed origin/dev red (task d5808cd1). The reverted change was chasing a real defect: that node had 27 resume-actuator dispatches, repeated exit=-9 kills at 1800.2 s, and a litellm ContextWindowExceededError at 133,278 tokens, and it did park the task blocked. But joining the allowlist does not fix the envelope, it makes the node skip its work. write-failing-tests-loop.json declares the prompt as Draft a failing test (do NOT write it to disk, this is a DRAFT only), so a dispatched write_failing_tests reports a draft and advances while NOTHING writes the tests-only commit. That commit is what the red seat anchors to, so the change would have made red_gate undemonstrable for every task in the repo, not only this one, which is the fast-and-green-on-nothing outcome 7.13.288 and 7.13.290 both named in advance. Two guards said so before the fact and were not run: tests/unit/test_verify_plan_node_runs_its_declared_plan.py::test_the_three_build_steps_stay_out_of_the_allowlist failed on the pushed commit with write_failing_tests does heavy work; its declared prompt only DRAFTS it, and the reverted diff deleted the source comment carrying the same warning. THE DEFECT IS STILL OPEN and stays with task d5808cd1: the honest repair is the envelope or the declaration, never the allowlist. Three candidates, none yet evidenced: trim what the full brief assembles so the step keeps its tools and still writes and commits; or change the behavior declaration itself so this node declares a real write-and-commit prompt rather than a draft; or attribute the 1800 s to the single engine slot (launch.sh --parallel 1) queueing the step behind live drives rather than to the model. A change to a shared allowlist runs every suite that reads it first: grep -rln _PLANNED_STEPS services/prism-service/tests/. '
    '7.13.290: the declared flow RUNS as steps; it was being read as a settings file (task eda5a843). _node_plan opened a behavior JSON and harvested scalars from it, the route NAME, model, turns, budget, timeout and prompt, and never called a declared step. Every non-agentic route went into a codified list that exactly two hardcoded branches read by name, premise-gather at task_runner.py:1445 and premise-citation-check at :1542, so verify-plan-loop.json step[1] text-challenge was collected and dropped. The canvas drew a flow and the daemon ran a parallel Python implementation of it. Same shape elsewhere: ship_worker.py:742 calls task_reaper.reap_task directly while POST /api/workflows/steps/reap is declared, drawn and never dispatched. THE COST WAS NOT COSMETIC. reason-loop is where Observe (the ContextBuilder bundle), the json_schema constraint and Validate (the plan_coverage rubric) live, at api/workflows.py:1941-1984. Lifting only its PROMPT out and calling claude_cli directly kept Reason and dropped the other three, so a verify_plan artifact came back as raw prose carrying an empty think wrapper and a narrated file read instead of a schema-constrained plan_doc plus plan_diagram object: the schema that forbids a chat transcript was declared and never applied. _node_plan now returns the steps whole and in file order, with the declared json_schema and rubric beside them, and a generic _dispatch_declared_steps runs each one. Dispatch is IN PROCESS, never a self HTTP call: this worker runs inside the service that serves the routes, and the engine behind an agentic step serves one request at a time, so a worker that POSTed to its own API could wait on a slot it is itself holding. Bodies are templates, so ${taskHint} and ${taskId} are filled before the step runs. A route with no handler is REPORTED ok false with a reason, never skipped, because silence is what let text-challenge sit declared and unrun for versions. WIRED NARROWLY ON PURPOSE: only verify_plan dispatches, guarded by _runs_as_declared_steps, and a parametrised test pins that write_failing_tests, implement_tasks and verify_green_state never will, because their declared prompts only DRAFT the work and dispatching them would make a drive fast and green on nothing. A chain that returns no document falls back to the inline call, so a bad declaration is never worse than before. ReasonLoopRequest gains narrow, default true, which passes an empty allowed_tools: every declared agentic middle on this bot is a no-tool text generation, and leaving tools on would have silently undone the 7.13.289 envelope win, 28,560 to 8,159 input tokens on this very step. NOT YET OBSERVED LIVE: 13 new tests pin the wiring and the fallback, and the artifact improvement this predicts, a schema-constrained document with no think wrapper, has not been watched on a running daemon. Full trees at this tree: tests/unit 4424 passed with 5 pre-existing failures reproduced identically at 31050f31 (brain_csharp_calls x3, install_packaging_assets x2), tests/integration 787 passed with 3 pre-existing failures (the web_dist no-built-assets pair, and a headless-drive claim collision with the live daemon that reproduces on base). '
    '7.13.289: a no-tool node call no longer inherits an interactive agent CLAUDE.md. MEASURED on the live daemon 2026-09-10. The declared verify_plan middle is a 1295-character prompt with no tools, and its call carried 28,560 input tokens. Decomposed with three claude -p runs whose flags matched the live seat exactly, varying only what the child could see: a trivial 30-character prompt in the real task workspace cost 28,003 input tokens and 125.1 s, the same prompt in an empty scratch directory cost 7,602 and 33.3 s, so the workspace itself is 20,401 tokens. About 98 percent of that node input is envelope, present whatever we ask. The workspace carries a 75,961-byte CLAUDE.md written for an interactive coding agent. Given about 20k tokens of you-read-files-and-run-commands and then --tools with an empty value, the model role-plays the loop anyway: task d5808cd1 plan_doc opens with an empty think wrapper, then Let me read the file to understand the context, then a file-read block whose contents are INVENTED, naming prism_service/tasks/reap.py which does not exist. 7.13.287 removed the tools; only this removes the belief that it has them. THE FIX IS THE CWD: a call with an empty allowed_tools now runs from an empty stable directory under the data dir, and --plugin-dir moves with it, because each carries its own discovery. NOT --bare, which also skips CLAUDE.md discovery but declares that auth is strictly ANTHROPIC_API_KEY or apiKeyHelper while _strip_env removes that variable under INV-1, so it would risk the local path and break every user who is not on an API key. Discovery is directory-driven, which the --bare help confirms by offering --add-dir for CLAUDE.md dirs. The directory is STABLE rather than a per-call tempdir on purpose: the cwd appears in the harness system prompt, so a path that changed per call would change the prefix per call and defeat prompt caching, and caching is what makes the envelope cheap when it hits (a draft_story run paid 613 fresh tokens against 27,520 cache_read and finished in 23.9 s). Six no-tool call sites inherit this: brain ask, graph_enrich, memory_summary_worker, event_handlers, the premise-judge route, and the narrow declared middles in both driving seats. Tool-using steps are untouched and keep the task workspace and its CLAUDE.md, pinned by a regression test. CAVEAT STATED PLAINLY: this is a QUALITY fix, and the latency claim is not yet settled. A 125.1 s figure for a 30-character prompt is the cost of a cache MISS, not a per-call tax; the envelope does cache, and a hit costs 613 fresh tokens. Whether the miss is caused by single-slot eviction or by prefix ordering is unresolved and is being measured separately. '
    '7.13.288: verify_plan runs its declared narrow middle, and a timeout stops calling itself a crash. MEASURED on the live daemon 2026-09-10, same model, same CPU, same box: draft_story on its narrow declared plan with zero tools took 23.9 s and went rubric-green on its own, while verify_plan on the full brief with BUILD_TOOLS was killed at 900.1 s. Manifest row 4ceabf19baf6: tokens_used 0, cost_usd 0.0, model empty, final_text the model empty think wrapper, stderr claude -p timed out after 900.0s. So the discriminator is what we send the model, not the model and not the clock. verify-plan-loop.json has declared a real narrow middle since v3 and nothing read it, because _node_plan returns None outside _PLANNED_STEPS. verify_plan now joins that set WITH a _declared_agentic_prompt branch: the allowlist entry alone would only buy a faster failure, since _invoke_budget adopts the declared caps only when the declared prompt is in play. The prompt is READ FROM THE DECLARATION rather than copied into task_runner, so the behaviour file cannot drift from the canvas, and _node_plan now carries the prompt beside the caps so the two always travel together (task 6a7105f9). The three BUILD steps stay out on purpose and a test now says so with their own words quoted: their declared prompts open Draft a failing test do NOT write it to disk, Draft an implementation approach do NOT write any code, and OBSERVE-only check, while those steps must write the tests (the tests-only commit is the red seat anchor), change the code and run the suite. Wiring them would make drives fast and green on nothing. PROOF SPLIT: _route_proof gains a verify_plan branch, because arc_governance reads the plan_diagram FIELD and refuses on missing or non-parsing, so one markdown blob into plan_doc would trade a timeout for a gate refusal. plan_doc keeps the whole report for AC coverage and plan_diagram gets the lifted mermaid source with fences stripped, matched against arc_governance own _MERMAID_KEYWORDS so extractor and judge agree. A report with no diagram leaves the field untouched: never fabricate what the rubric must be able to refuse. HONESTY GAP CLOSED, found live: 7.13.287 built the budget message only in the empty-proof branch, and the local model emits an empty think wrapper, so a run killed at its bound still returned NON-EMPTY proof and was reported as non-graceful failure crash auth truncated mid-turn. The exit code is now tested first and independently. BOUNDS SET FROM DATA, not from the template: computed over agent_runs healthy runs, draft_story n=326 p50 69.1 p99 361.7, verify_plan n=324 p50 128.1 p99 810.8. Declared bounds move 120 to 400 and 120 to 850, p99 rounded up. The 120 was a template default replicated identically across all six behaviour files and sits BELOW verify_plan healthy median, so the post-kill retry would have killed most healthy runs. draft_story three-attempt ladder is now 1700 s rather than 2700 s, correcting the 1140 s figure in 7.13.287 which came from the template number. A narrow-path measurement of verify_plan was attempted and DISCARDED: the daemon dispatched two more claude -p onto a one-slot engine mid-measurement, so it timed the queue rather than the model; that contention is filed as task 8b930a80. OPEN QUESTION, recorded rather than resolved: in one full tests/unit run on this tree, test_workflow_fix_request::test_the_result_names_where_its_evidence_lives failed while passing alone, and it asserts a git workspace baseline in code this change never touches. A pairwise run of it beside both new test files in one invocation is green (24 passed), so the new files do not pollute it. The failure is NOT attributed. Untested hypothesis for a later session: a second full tests/unit run was live on the box at the time, pid 200695 from /home/siegeon/.claude/jobs/qa-verify-71714987/, a job that was also driving a task, and ensure_workspace defaults base_ref to the repository CURRENT HEAD. A full-suite result on this machine is not reproducible at present unless you know what else was running. '
    '7.13.287: a no-tool node call now really runs with no tools, and a budget kill says which budget it exceeded. 7.13.286 read the live child argv and found the resume actuator handing a declared no-tool middle the full BUILD_TOOLS set. It passed allowed_tools as an empty tuple to fix that. The empty tuple was the trap: claude_cli._build_cmd treated a falsy tuple as omit the flag, so the child got claudes DEFAULT toolset AND every configured MCP server. The system init event of run a141a41ee2ae, a task-runner draft_story call on task d5808cd1, shows 40 plus built-in tools and 13 MCP servers offered to a single no-tool text generation. The model spent the whole 900 s in mcp__prism__brain_ round trips and never emitted a result event. PRISM recorded this symptom once before, in v5.3.14, and worked around it in the prompt instead of on the command line, so five later call sites inherited the wrong belief: premise-judge, brain ask, graph_enrich, and the narrow declared middles in both driving seats. _build_cmd now emits --tools with an empty value plus --strict-mcp-config for an empty tuple. Verified live against claude 2.1.267 in the d5808cd1 workspace: the init event reports tools [] and mcp_servers []. Second fix, honesty: exit=-9 is claude_cli.TIMEOUT_EXIT_CODE, PRISMs own timeout when subprocess.run raises TimeoutExpired, not a host SIGKILL. Reported as exit=-9, no usable output it read as a crash, which is how d5808cd1 seven timeouts were first diagnosed as an out-of-memory kill on a host with about 60 GB free and no OOM row in dmesg. The reason now names the limit and the elapsed time. Third fix, cost: _node_plan reads the timeoutSeconds a node declares, and a retry whose previous outcome at that step was a budget kill runs on that declared bound instead of a second full budget. draft-story-loop declares 120 s, so its three-attempt ladder costs 1140 s instead of 2700 s. Both driving seats share it. 7 new tests in test_stall_retry_budget.py, and the two suites that pin the word KILLED in the stall row stay green: the kill wording is the invariant, the false attribution to the host and the raise-the-timeout advice are what changed. '
    '7.13.286: the resume actuator seat obeys a declared node plan, and its children get a wall clock. 7.13.284 taught task_runner to run a node declared plan. It did not teach prism-resume-actuator, which is a SECOND driver with its own claude_cli.invoke, so every task this seat drives kept the old path. Read off the live child process argv on 2026-09-10 while the daemon drove task d5808cd1 at draft_story: max-turns 30 against a declared 4, max-budget-usd 2.00 against a declared 0.50, allowedTools the full BUILD_TOOLS set against a declared middle that takes none, no model flag at all against a declared haiku, and the whole step brief as the prompt instead of the declared narrow one. Every cap shipped in 7.13.284 was bypassed on the running system. The seat now calls the same _node_plan, _declared_agentic_prompt and _invoke_budget helpers task_runner uses, and passes the result to invoke. It also passes timeout_s for the first time: claude_cli.invoke declares timeout_s float or None with a None default, and this seat passed none, so its children had NO wall clock and one dispatch was observed past 20 minutes. A step with no declared plan is unchanged: the full brief, BUILD_TOOLS and the runner defaults, and it now gains a timeout too. 3 new tests in test_resume_actuator_runs_the_declared_plan.py assert on the kwargs the invoke actually received, plus 20 existing actuator tests green. Also in this build: write_failing_tests joins _SLOW_STEPS at 2x, because it writes tests AND runs them like its two siblings while getting the flat 900 s. Measured over sane agent_runs rows: p90 1528 s against that wall, where implement_tasks p90 1426 s is covered by its 2x and verify_green_state p90 1759 s by its 3x. verify_plan is deliberately NOT added, p90 663 s is already under the wall. CORRECTION to the 7.13.284 note, which is edited in place: its draft_story figures came from agent_runs.duration_ms unfiltered, and that table also holds seat rows measured in days, the largest four being 61, 48, 45 and 15 days. Filtered to rows under 4 hours the real figures are 337 runs, median 71 s, p90 255 s, and 10 runs at or past 890 s, which is 3.0 percent and not the 6.3 percent first published. The fix itself rests on task d5808cd1 own three exit=-9 kills, whose stall rows name the 900 s budget in words. '
    '7.13.285: an operator who fixes the cause can restart a stalled task (task 1ecbd866). _stall_count counted non-advancing attempts since the most recent rewind only, and STALL_ATTEMPTS is 3, so once a step spent its budget _run_one_step routed every later tick straight to _handle_stall and never called the step again. Nothing an operator could do reset it. Measured live on task d5808cd1: after the draft_story fix shipped in 7.13.284, a click on the task page moved it blocked to in_progress at 02:56:39 and prism-task-runner blocked it again at 02:56:41, two seconds later, without attempting the step at all. A move out of blocked is the same fresh mandate a gate rejection already is, so it now ends the budget window exactly as REWIND_ACTION does. The match is DIRECTIONAL on purpose: TaskService records the transition as an action=updated row whose details read status: blocked -> in_progress, while the BLOCKING row reads status: in_progress -> blocked and contains both words, so a substring test would reset the budget on the very row that blocks the task and retire the 3-attempt guard altogether. No history row is deleted or rewritten and STALL_ATTEMPTS is unchanged, both of which the ticket names as misfires. Known gap, filed separately: the ticket also asks that the retry carry an ACTOR, and TaskService writes none on that row (task_service.py:1322), which needs a file outside this slice. 2 new tests in test_a_resolved_stall_lets_the_task_retry.py, one of them driving a REAL TaskService rather than synthetic history rows so it cannot drift from the recorded format, plus 57 green in the six neighbouring runner suites. The direction guard was verified by mutation: reverting to a substring match turns it red. '
    '7.13.284: the drive obeys the draft story node plan, and a stall names its own step. The conductor declares draft_story in .prism/behaviors/conductor/draft-story-loop.json as haiku, 4 turns, 0.50 USD, no tools, with a prompt that names the three headings the story_complete rubric scores. task_runner._PLANNED_STEPS admitted only review_previous_notes, so _node_plan returned None for draft_story and the worker sent the whole step brief at 30 turns, 2.00 USD and 900 s with the full BUILD_TOOLS set. Measured over 337 recorded draft_story runs in projects/prism/scores.db (filtered to under 4 hours; the table also records seat rows measured in days): median 71 s, p90 255 s, and 10 runs (3.0 percent) at or past 890 s, where the step is SIGKILLed and records no agent_runs row at all. Task d5808cd1 hit that tail three times, then exit=1, and blocked with an empty story_md. draft_story now joins _PLANNED_STEPS and runs its declared narrow prompt with the caps that prompt was sized for, never the caps alone (task 6a7105f9 proved that shape dies exit=1). _run_one_step now loads the task for ANY declared plan, not only when premise-gather is codified, because draft_story declares text-challenge instead and would otherwise render a prompt with no task material and no tools to find it. The narrow prompt refuses an empty task hint so the full brief stays the honest fallback. Second defect, same path: _codified_red_test_ids is a RED-TEST read that the stall consulted at every step, so a draft_story stall told the owner that write_failing_tests had landed no tests-only commit, three steps before any test exists. A new _RED_TEST_STEPS set scopes that read to write_failing_tests, implement_tasks and verify_green_state; the SIGKILL branch still outranks everything at every step. 14 tests in test_draft_story_node_runs_its_declared_plan.py, 2 of them pinning the live dispatch path, plus 43 green in the five neighbouring suites. Task 60ede4d9. '
    '7.13.283: terminal tasks close via the gate_adjudicator sweep, not just on flow_report. A task sitting at workflow_step=done with no driver was never reaching flow_report to trigger closure. Now gate_adjudicator.sweep_once runs _close_if_terminal on every task at a done step with no pending/failed gate, using backoff to avoid re-attempting unreachable tasks. The check runs BEFORE the gate-step filter, so terminal tasks close and then skip gate logic. Task edeab040 sat at done status=in_progress forever; the sweep wakes it every 60 seconds and closes it on first pass with no driver needed. New tests verify the sweep path and backoff use. Existing flow_report hook remains as a fast path for tasks terminating while a driver is attached. 16 terminal closure tests, 704 gate/adjudicate tests green. '
    '7.13.282: terminal steps now close tasks automatically. When a task advances to a step with type=done and no gate is outstanding (gate_state not pending/failed), the task closes with status=done and completed_at set to current UTC timestamp. This is workflow-agnostic: the condition checks step type and gate state only, never the workflow name. Triage reaches done only after decide passes; implement reaches done only after green_gate passes. Task_runner.eligible_tasks now skips terminal steps so they are never driven by the executor. conductor_flow._close_if_terminal hooks after every advance and gate decision. New test suite test_terminal_step_closes_task.py with 12 cases covering both workflows and all gate states. Task edeab040: reached done but never closed. '
    '7.13.281: the triage decide seat now accepts failed gates for re-sweep. gate_adjudicator.sweep_once checks _failed_gate_is_refused_approve to distinguish human rejects from machine refusals, then calls adjudicate on both pending and failed gates. adjudicate was only accepting pending gates, discarding failed-but-resweppable ones. Now accepts gate_state in (pending, failed); passed gates stay decided. New tests verify the four cases: failed-resweppable, pending, passed, and wrong workflow/step. Comment in code names the upstream guard so future readers do not widen this further without it. Task edeab040. '
...
# NOTE (7.13.284): this was a bare `=`, which silently CLOBBERED the block
# above it -- 7.13.283, .282, .281, .280, .279, .278 and .276 were all
# written into a variable that the very next statement threw away. Every
# entry from .283 back to .276 was dead the moment it was added. `+=` keeps
# the chain intact; the entries above are now reachable again.
PRISM_VERSION_NOTES += (
...
PRISM_VERSION_NOTES += (
    "\n7.13.255: main's PR checks were red for every task, so ship_worker's "
    ...
    "the machine-local PRISM evidence store; its own docstring says the file "
    "is throwaway and must never merge to main, and its fixture task 31106380 "
    "no longer exists. Deleted. (3) test_behaviour_substeps_show_their_runs.py "
    "drove _conductor_behavior_workflows, which returns an empty list when the "
    "project source path is absent from disk, before the patched engine is "
...
PRISM_VERSION_NOTES += (
    '7.13.309: 7.13.308 shipped the backend, and the Workflows canvas still drew the occupied node the same as an idle one. GET /api/workflows for implement-tasks-loop already answered occupancy loop 1, text-challenge 0 for a real live drive on task b490fabc, and workflowGraph.ts already carries the paint (a shadow blur glow, a 2.5px accent stroke, an agent glyph, task 67a98810) for exactly this drilled sub-flow case. Two screenshots a few minutes apart still rendered loop and text-challenge identically. Root cause is in WorkflowsPage.tsx, not the canvas: the definition and occupancy poll only re-applies a fresh def when viewingInstanceRef.current is false, and that ref is a single global flag set true by replayHistoricalRun for either of its two callers, a conductor rail pill or an attached task instance. Both callers only ever write synthetic per-step occupancy onto a FLAT root canvas conductor or validation, so the freeze exists to stop the live poll from stomping that synthetic occupancy. Landing on workflows with a task param and a workflow param naming a nested behaviour sets that same global ref true for an unrelated reason, the top level task instance effect fires regardless of which layer is selected, and nothing then ever re-applies live occupancy to the nested layer again: it is frozen at whatever it read on the one race-prone tick before the freeze, for as long as the tab stays open. Fix: the poll now also refreshes whenever the selected catalog entry carries a parent_id, since a nested behaviour never receives synthetic occupancy from either replay path, so refreshing it live is always safe. 1 new test in test_layer_nodes_show_their_real_state.py reads the poll block itself and pins both the OR and that the flat-canvas freeze is relaxed, never removed.'
)

PRISM_VERSION_NOTES += (
    '7.13.310: the Workflows canvas shows the processing on each working node as a task crosses to completion, and a person can lift a dispatch-guard park. Owner direction 2026-09-12: PRISM is a game and the workflow page is where the owner watches it play in real time; that page was not real. Four slices, built in parallel by four agents outside the task flow and integrated once. (1) drive_heartbeat rows gain an additive node column: task_runner._dispatch_declared_steps beats node=<route> immediately before each declared sub-step handler and node= (empty) after the loop, and the reason-loop route beats at entry and exit when the body names a task, so the row always names the declared node executing right now; a beat whose node changes counts as progress even when work_units repeats. get_workflows lights the behaviour step whose route matches the live beat node and falls back to the entry node when the node is empty or unmatched, still with exactly one latest_many call per request. (2) workflowGraph.ts paints an occupied behaviour node as RUNNING with an elapsed clock in place of the stale run-history line, plus a gentle sine-driven glow pulse (steady glow, no pulse, under reduced motion); active-driven and verdict-driven paint is untouched. (3) POST /api/conductor/park/release dispatches by blocked_reason prefix to dispatch_guard.release or resume_actuator.release, and answers 409 for any block that is not a governance park; the dispatch-guard release had no HTTP route at all, so parked for a person had no person-facing action, verified live on task b490fabc at 12 of 12 dispatches. (4) the task page renders Release - try again inside the BLOCKED banner for a governance park and disables the generic in_progress transition while one is present, with an inline refusal text on 409. Also re-anchored test_workflows_section_ui poll-freeze pin, which indexed the pre-7.13.309 guard literal and left main red. 17 new tests across 6 files; npm run build green on the merged frontend.'
)

PRISM_VERSION_NOTES += (
    '7.13.311: a person releases a parked task once, not twice. Measured live at 06:15-06:17 UTC on task b490fabc, minutes after 7.13.310 shipped the Release button: the click released the dispatch-guard park, task_runner dispatched at once (dispatch 1 of 12), and the resume actuator immediately re-parked the same task at ITS ceiling (13 of 12), because the two governance seats keep two independent dispatch counters and the release route only reset the seat named in the current blocked_reason. The owner had to click Release a second time before the drive held. park/release now calls both dispatch_guard.release and resume_actuator.release, primary seat first: each seat writes its own released history row unconditionally (that row is what resets its counter) and only flips status when the park is its own, so the sibling call resets the counter and touches nothing else. Response lists released seats and names the primary; module stays as an alias the task page already reads. 409 for a non-governance block and 422 for a missing task_id are unchanged. Live proof of the whole chain in the same window: after the release the loop node on the implement-tasks-loop canvas read RUNNING with a clock ticking 18s then 1m 34s on the owner tab, and the drive heartbeat beat every 60s.'
)

PRISM_VERSION_NOTES += (
    '7.13.312: visible truth on the game board, a 1-second node-to-view link, and no path from a local-backend drive to Anthropic. Four slices built in parallel outside the task flow and integrated once, each from a defect measured live on task b490fabc between 06:14 and 06:47 UTC. (1) THE GHOST BEAT: resume_actuator.dispatch_once beat the drive heartbeat and charged its 12-dispatch ceiling BEFORE acquiring the claim, then deferred with already driving held by prism-task-runner; task_runner read that beat as a live driver and skipped the task; the canvas painted the node RUNNING for 30 minutes on a seat that only checked, and a lease held by a process the daemon restart had killed blocked both seats until it expired. The seat now acquires the claim first and beats only while it holds it, a deferred attempt writes no beat and no dispatch row, ClaimService gains release_by_holder, and main.py lifespan releases both seats own leases from a previous process before either thread starts. (2) VISIBLE TRUTH: get_workflows emits live[step] with task_id, driver, tool, node, age_s, since and dispatching, where dispatching is true only for the open-dispatch tools dispatch_guard_live, claude_cli.invoke and, now that it beats only after the claim, resume_actuator_dispatch; the canvas paints WAITING - tool, dim and unpulsed, for any other beat, and RUNNING - elapsed - driver - tool for a real one, with elapsed from the server since. (3) FAST LINK: GET /api/workflows/live answers from one task listing and one latest_many in about 10 ms on the real 54-task set; useWorkflowLive polls it every 1000 ms while the tab is visible, pauses when hidden and backs off after failures, and overlays occupancy and live onto the selected layer under the same freeze guard the 10-second catalog poll uses, so a node reaches the screen in about a second instead of 20 to 60. (4) NO ANTHROPIC: a live claude -p child of the daemon carried the right local env and still held an ESTABLISHED TLS connection to 160.79.104.10:443 with SNI api.anthropic.com, because the CLI own OAuth session in the inherited config dir drives keychain, connector and usage prefetches that ANTHROPIC_BASE_URL never gates. Under the local backend every child now gets a PRISM-owned credential-free CLAUDE_CONFIG_DIR with the work dir trust pre-seeded, and invoke refuses by name, before any spawn, when the backend is local and no base URL is configured; MCP tools still reach the child from the workspace .mcp.json. The 18.11 dollar spend card was a separate pricing-table gap, not egress, and is left as is. 43 new tests across nine files; the merged frontend builds green.'
)
...
PRISM_VERSION_NOTES += (
    '7.13.316: a shipped build reaches the dev instance without a hand (task 13cfe8ee). Owner 2026-08-27: a lot of this flow should have been part of the conductor flow in the app, not up to you -- a session did the same four steps by hand four times: git pull in the shared checkout, a status check, npm run build of web_dist, an aspire restart, poll /api/version. A new deploy_worker.py seat sits next to ship_worker.py: ship_task calls deploy_after_land once a branch is on origin/main, off by default behind PRISM_DEPLOY_ON_LAND. deploy_once refuses with a reason when a POLICY_FILES entry (or anything else) is uncommitted in the daemon own checkout, else runs PRISM_DEPLOY_COMMAND when set or else fetch, ff-only pull, a web rebuild only when the pulled range touched a web file, then auto_updater.request_restart -- the existing issue 66 safe handoff, never an exec of its own. Because that call does not survive the restart it just asked for, the pipeline records a requested history row naming the target version, and confirm_pending_deploy resolves it to confirmed or parked on a LATER tick, including from the fresh process after the real restart; a deploy-worker sweep thread (also opt-in) and a POST /api/deploy/run trigger both call it. New steps/deploy and steps/deploy-verify routes in api/workflows.py back a deploy node nested under the conductor the same way land and reap already are, declared in .prism/behaviors/conductor/deploy.json (not live-verified against the AosWorkflows engine, same caveat as the 7.12.39 land.json note). conductor-deployer is a registered machine seat. 10 tests in test_deploy_seat.py, committed tests-first and proved red (ImportError) before the implementation landed; git runs for real against throwaway repos the suite builds, npm and the restart/version-fetch boundaries are fakes. 433 tests across every suite reading ship_worker.py, actor_service.py, and the conductor behaviour catalog stay green.'
)

PRISM_VERSION_NOTES += (
    '7.13.317: the daemon stands down when a session is already driving a task (tasks ad38e421, 7180de77). dispatch._drive_now calls task_runner._run_one_step directly, the instant a step advances (after_step) or a task first goes in_progress (on_started) -- bypassing eligible_tasks, the only place that checked task_runner._foreign_driver_on before this fix. A session driving a task through conductor_work posts a drive heartbeat exactly like the runner does, under its own driver name, so a live one was already treated as somebody-elses-task everywhere except this instant path. Measured live on 4d86db87 (a sessions 8-AC story overwritten within 2s of a daemon drive) and 87cb620e (a think preamble written into plan_doc mid-drive). Fix: dispatch.py gains _foreign_driver_on, a thin delegate to task_runner own check (one definition, not a second copy that can drift); after_step and on_started both consult it before calling _drive_now and return drove=false with a reason naming the live driver when one is present. task_runner._run_one_step also checks the same guard at its own top, before flow_start, before its own heartbeat, and before dispatch_guards chokepoint -- so a refused drive never reports a step (plan_doc cannot be clobbered) and never writes a dispatch_guard_dispatch history row (no real dispatch was attempted). 6 new tests across test_handoff_respects_a_live_session_driver.py and test_runner_respects_live_drives.py, committed tests-first and proved red (AttributeError / a bare invoke) before the fix landed. Full unit suite: 4691 passed, 6 pre-existing unrelated failures (C# call-graph, packaging-wheel assets, workflow_fix_request snapshot -- none touch dispatch.py or task_runner.py). Integration: 788 passed, 2 pre-existing failures needing a web_dist build this fix does not touch.'
)

PRISM_VERSION_NOTES += (
    '7.13.318: a machine seat no longer re-approves a rejected plan unchanged (task c2e6edaf). Observed live on task d0b392b3, 2026-09-12: a distinct reviewer rejected plan_gate at 18:09:25 naming six findings, including invented AC ids and files that do not exist; plan_doc did not change; at 18:11:36 the conductor-adjudicator certainty seat approved the same text anyway, because task.oracle, an unrelated field, gained backtick paths and moved the score from 0.88 to 1.00. Two defects, one fix: neither machine seat ever asked whether a human had just refused this exact plan. A fourth deterministic plan tooth, plan_gate_checks.manual_reject_stands, declared as a real node in .prism/behaviors/conductor/plan-gate-check.json, reads the latest human gate_decide reject of plan_gate from task history and compares the plan_doc content hash conductor_service.py now stamps onto that reject row against the current plan_doc; while they match, every seat that reads plan_gate_checks.refusal declines before the certainty seat is ever reached, and GET /api/conductor/gate/readiness declines and names who rejected it, when, and the first finding, ahead of the design-packet approval-status branch it used to answer with instead. The moment plan_doc changes, the hash no longer matches and the seat may decide again; a child task is untouched, since it already clears on the rubric alone. Pinned by test_plan_gate_respects_manual_reject.py (services/prism-service/tests/unit), committed tests-first: 12 of 12 red against the base commit, all pass after the fix. Also rescopes two neighbouring pinned tests this same change legitimately shifts: test_task_score.py::test_the_slice_edits_no_policy_file (was an unconditional branch-diff check with no escape hatch; now checks task_score.py itself names no POLICY_FILES module) and test_workflow_layer_instances.py (plan-gate-check.json current_version 3 -> 4, the new manual_reject_stands step).'
)

PRISM_VERSION_NOTES += (
    '7.13.319: a plan_gate/story_gate whose own rubric already fails never parks for a person to judge it (task 3feaf956). On 2026-08-28 task 12029f92 reached plan_gate five times; each time the real arc_governance scorers already knew the story was missing required sections and the plan covered none of its ACs, and nothing ever bounced it back before a human looked at it. ConductorService.rubric_gate_failure_outcome decides REWIND vs PARK from the ONE check api/conductor_flow._autoclear_machine_gate already computed (never re-scored, this ticket own likely_misfire against doubling gate latency): a genuine rubric failure rewinds to the producing step (verify_plan/draft_story) with the scorer own reason riding the next job instructions, the same shape a manual gate reject uses; a SCORER ERROR (the rubric function itself raised) still just parks, unchanged, per this ticket own first stop_if against looping a task behind a broken scorer. The second stop_if, verification fails twice, is its own guard: a new _last_rubric_rewind_reason scans the FULL task history (not a recency window, since a fresh advance_task row sits between every pair of rewinds) for the most recent auto_rewind FROM this exact gate; an IDENTICAL reason recurring parks instead of rewinding again, so a producing step that resubmits unchanged content -- exactly what test_worker_contract_enforced.py and test_conductor_work_honest_green.py drive through their fixtures -- cannot bounce a task between the gate and its producing step forever. A DIFFERENT reason still earns another rewind. Two pre-existing pins retired the OLD park-on-failure contract this ticket replaces (test_gate_autoclear_and_recheck.py), one adjusted its setup to keep covering the unrelated manual reject/recheck cycle it actually probes. 15 tests across test_gate_never_parks_on_failing_rubric.py and test_gate_autoclear_and_recheck.py, committed tests-first and proved red against the pre-fix commit (4 failed / 4 passed) before the implementation landed. Full unit suite: 4713 passed, 6 pre-existing unrelated failures (C# call-graph x3, packaging-wheel assets x2, a flaky ship_worker test that passes in isolation) -- none touch conductor_service.py or conductor_flow.py. Full integration suite: 787 passed, 3 pre-existing failures (a web_dist build this fix does not touch, and one flaky already-driving daemon-lock collision) -- identical to the origin/main baseline before this branch.'
)

PRISM_VERSION_NOTES += (
    '7.13.321: PR #5972 (task d0b392b3) sat FAILURE across 8 straight CI runs even after the 7.13.320 CI fixes landed on main -- not a diff-of-its-own defect and not behind main (already carried 670bcb41). Root cause: tests/unit/test_deploy_seat.py::_land_from_elsewhere clones origin AFTER main already exists there; origin bare repo own HEAD symref was set at git init --bare time, before main was ever pushed, so it still names an unborn ref (git builtin fallback "master") a plain clone cannot check out. Any dev box whose global init.defaultBranch already reads "main" (this sandbox included) masks the bug, since the clone default happens to coincide; GitHub Actions ubuntu-latest sets no such override, so the second clone landed on a fresh unborn master and git push origin main failed with "src refspec main does not match any" -- reproduced identically in a clean ubuntu:24.04 container and locally by forcing GIT_CONFIG_GLOBAL=/dev/null, GIT_CONFIG_NOSYSTEM=1 for the test run. Fix: _land_from_elsewhere checks out main explicitly after cloning (origin/main was fetched during clone regardless of the HEAD-symref gap). Separately, ship_worker.ship_task now names the actual failing TEST(s) on a genuine (non-pending) ci_wait failure, not only the failing job/check: _ci_failure_test_detail pulls the run id out of the checks row own /actions/runs/<id> URL and greps gh run view --log-failed for FAILED <nodeid> lines, appended to the verbatim gh pr checks text (never replacing it, never blocking parking on any miss -- no run url, gh unavailable, no matching lines all leave the message unchanged). This is what closed the actual 8-run loop: a driving agent no longer has to open the Actions log by hand to learn which test broke. The behind-main half of this same class was already handled -- ship_task rebases onto origin/main before every push and self-heals a version-only conflict; nothing there needed a change. 2 new tests in test_ship_worker.py (test_ci_wait_failure_names_the_failing_test, test_ci_wait_failure_without_a_run_url_is_unchanged), committed alongside the fix and proved red by reverting the detail lookup in place (1 failed as expected, the miss-path test correctly stayed green) before restoring it. test_deploy_seat.py: 10/10 passed under both this sandbox git config AND the CI-like forced config. test_ship_worker.py: 20/20 passed.'
)

PRISM_VERSION_NOTES += (
    '7.13.320: three CI-only unit-test failures fixed (task 4465ee72), each an environment gap invisible on a dev box. (1) test_resume_actuator_runs_the_declared_plan.py: task_runner._behavior_dir falls back to Path.home()/projects/<project> when the configured source path is not absolute or does not exist, and that fallback only ever resolved on a dev machine whose home directory happens to hold a real checkout there -- on CI it read nothing, so draft_story dispatched with model empty instead of the declared haiku. Test now carries the same hermetic _behavior_dir monkeypatch already used by test_task_runner_honors_declared_node_plan.py and test_draft_story_node_runs_its_declared_plan.py (task a2bc8c88); no production code changed. (2) test_workflow_fix_request.py: the test doubles _TaskSvc.create returned a deterministic f"task-{len(self.created)}" id, so two unrelated repairs queued in the same pytest session could collide on the same id; task_workspace.ensure_workspace then self-healed the later call onto the earlier on-disk workspace record whenever both landed inside the same wall-clock second -- a real git worktree add is slow enough that a CI runner crosses that boundary where a dev box usually does not. Switched to a real unique id (uuid4) and added test_two_repair_tasks_never_share_a_workspace, which forces the collision deterministically with two separate _TaskSvc() instances rather than relying on timing. (3) test_claude_cli_child_dies_with_parent.py: pins the pdeathsig arm-vs-check race, the known loaded-runner orphan flake named in task 8b4073df. The setpriv prctl(PR_SET_PDEATHSIG) call is a plain step early in its startup, not atomic with fork -- if the real parent dies in that window the signal is never armed and the child is a permanent, unsignalled orphan; the window is microseconds under normal scheduling but was measured past 15 seconds under synthetic CPU load. man 2 prctl documents the standard closure -- recheck the parent with getppid() immediately after arming -- so _with_parent_death_signal now hands setpriv a second exec stage, sh -c \'if [ "$PPID" = "<expected>" ]; then exec "$@"; fi\', which reads $PPID fresh at its own startup and only execs the real payload when the parent pid still matches; a parent that already died, whether before or after the signal armed, leaves the shell to exit without ever spawning the payload. Tests-only commit landed first and proven red against unfixed code (2 failed on the guard-shape assertions, 15 passed); the production fix then made all three files, and the ticket oracle command (HOME=<empty>, GIT_CONFIG_GLOBAL=/dev/null, CI=true) green. Full unit suite: 4700 passed, 12 skipped (one order-dependent pre-existing flake in test_handoff_respects_a_live_session_driver.py, observed once across three runs, reproduced with no code under this task touching dispatch.py, task_runner.py or drive_heartbeat.py, and absent on a fresh unmodified origin/main run of the same file). Integration: 787 passed, 3 pre-existing failures confirmed identical on a fresh origin/main worktree (2 need a web_dist build this task does not touch, 1 is the already-documented lease-timing race from 7.13.315 and 7.13.317) -- none are in this ticket verify list.'
)

PRISM_VERSION_NOTES += (
    '7.13.322: the Workflows canvas shows a daemon-driven conductor task as genuinely running, and its node boxes stay legible at 1440x900. Observed live (screenshot ui-2-conductor-flow.png): /workflows?workflow=conductor rendered "No run in progress" while task a65c66e5 sat in_progress at plan_gate, gate_state=pending, actively driven by the daemon (owner: "real time progress moving items... it just looks exactly the same"); separately the 12-node canvas auto-fit its nodes down to an unreadable size at that viewport. Root cause of the first half: conductor_service.py activity_for deliberately reports "awaiting_gate", never "working"/"driving", for a task standing at a *_gate step with gate_state pending/failed (its own docstring: "a WAIT for review, not work") -- three surfaces on WorkflowsPage.tsx (liveRunning, conductorRowLiveness, and the canvas rAF loop own ambient node-highlight) each independently re-derived a narrower "activity.state working or driving" predicate that went blind for exactly this state, drifting from the SAME question the page own conductorRunGenuinelyActive and conductorPillTone already answered correctly. Fix: one shared conductorTaskGenuinelyActive(task: ManagedTask) helper (gate_state pending/failed OR activity working/driving), threaded through all three call sites; the status line running branch now also names the driven task and seat ("Driving <id8> · <step> · <seat>") instead of a bare "running · <step>". Second half: workflowGraph.ts fit() and the canvas own wheel-zoom-out handler shared a 0.35 zoom floor -- at that floor the 10-step conductor canvas 12px node label rendered at roughly 4px on screen. Both now clamp to a new exported MIN_ZOOM = 0.55; the canvas already supports drag-to-pan and wheel-zoom-in for whatever falls outside the initial fit. Verified live via the built SPA (vite preview) against the running dev daemon: the banner now reads "Driving 4c9b39e5 · green gate · prism-task-runner" and every node label is legible, screenshotted at 1440x900. New test_workflows_live_run_banner_ui.py (8 tests, tests-only commit proven red first: 8/8 failed against unfixed source) pins the shared helper existing and being used at all three call sites, the enriched status-line copy, and the raised MIN_ZOOM at both clamp sites. Full unit suite: 4726 passed, 12 skipped, 6 pre-existing unrelated failures (C# call-graph x3, packaging-wheel-asset x2, one sqlite "disk is full" under this host own 99%-full disk) -- none touch workflowGraph.ts or WorkflowsPage.tsx. Full integration suite: 787 passed, 3 pre-existing failures (2 already-documented task-runner spend/accumulator concurrency flakes, 1 more disk-full) -- none in this change verify list.'
)

PRISM_VERSION_NOTES += (
    '7.13.323: GET /api/work/graph no longer hangs on a large transcript corpus -- the /live page was stuck on "initializing..." for 30+s on the dev instance, and a direct curl -m 60 to /api/work/graph?project=prism never returned while /api/version answered instantly. Root cause: _task_spend_usd and the session/token-motion loop each call claude_transcripts.live_spend_for_session / live_token_events_for_session once per session per node with no wall-clock bound; on a cold cache each walks every directory under ~/.claude/projects (440 on the live instance) and parses whatever transcript files match (640MB+/358 files for this project alone) -- one slow session blocked the whole request, and N linked sessions blocked it N times over (the spend loop, then the session loop, call the SAME session again). Fix: a shared per-request deadline (_GRAPH_TIME_BUDGET_S, default 1.5s) plus a per-call timeout (_GRAPH_CALL_TIMEOUT_S, default 0.4s) via a small background executor (work.py._bounded/_GRAPH_IO_POOL). A call that cannot finish in time is skipped -- spend contribution 0, session absent from the graph -- for this one poll only, rather than blocking; the call itself keeps running in the background and warms claude_transcripts own (mtime,size)-keyed file cache, so a genuinely active session or task shows up again within a few polls once that read lands. _task_spend_usd(deadline=None) keeps the old unbounded behavior for direct/non-HTTP callers. 2 new tests in test_work_graph_transcript_io_budget.py, committed tests-first and proved red against unfixed code (6.05s and 24.12s against a 2.5s bound, with 1 and 4 slow sessions); both pass after the fix (3.69s combined). Neighbouring suites test_api_work_graph.py and test_work_graph_bounded_queries.py: 18/18 passed unchanged.'
)

PRISM_VERSION_NOTES += (
    '7.13.326: a task-agnostic worktree sweep reaps non-task worktrees (ops incident 2026-09-12) -- 173 registered git worktrees, disk at 98 percent, 93 of them landed and clean but invisible to task_reaper.reap_task, which only ever runs for the one task_id a land just finished. Agent worktrees (.claude/worktrees/*), QA/fixer worktrees and prism/ws/* branches whose task row was deleted have no matching task_id and were invisible to it forever. task_reaper.sweep_worktrees reads git worktree list --porcelain directly (never the workspace index) and reaps any worktree that is clean, at least 24h old (PRISM_REAP_SWEEP_MIN_AGE_H), unused (no process cwd inside it, via /proc), and whose branch tip is an ancestor of origin/main -- regardless of whether a task row exists. Unlike reap_task, this is the one place allowed to ask git directly whether a branch is an ancestor of main, because there is no task trailer here to misattribute shippedness to. Wired three ways: ship_worker._sweep_after_land runs it after every per-task reap (repo_root taken from the just-landed task own workspace record, resolved BEFORE reap_task pops that index row, and the sweep is skipped rather than guessed at a bare default when no repo_root resolves); task_reaper.start_worktree_sweep_worker is an opt-in periodic thread (PRISM_WORKTREE_SWEEP_INTERVAL, default off) filling the gap between lands; and a new sweep step in the reap.json behavior plus POST /api/workflows/steps/reap-sweep serve the manual and canvas entry point. Missing worktree directories are pruned either way; a branch is deleted only once its own worktree is actually removed and confirmed merged. Fails closed throughout: any error probing a candidate keeps it, never removes it on a guess. 15 new tests in test_worktree_sweep_reaps_non_task_worktrees.py, plus one existing pinned assertion in test_pipeline_reaps_a_finished_task.py narrowed from a whole-file is-ancestor ban to reap_task own call chain (sweep_worktrees legitimately uses is-ancestor; reap_task still never does). 28 reap/sweep tests green; full tests/unit suite 4744 passed, 5 pre-existing failures reproduced unchanged (brain_csharp_calls x3, install_packaging_assets x2).'
)

PRISM_VERSION_NOTES += (
    '7.13.324: the task detail page no longer 404s twice per load on a task whose driving session has not been transcript-parsed yet, or that has no archify map built (observed live on task a65c66e5-b8a7-44b4-a223-f1342cfaaa14: console showed GET /api/sessions/<id> -> 404 and GET /api/archify/maps/task?...task_id=a65c66e5... -> 404 on every poll). Root cause 1: GET /api/sessions/{id} only ever consulted session_outcomes (populated by transcript import), never task_sessions (populated the moment POST /api/tasks/{id}/sessions links a session id, which a background job can do before any transcript exists on disk for it) -- a linked-but-unscored session id therefore always 404d, breaking the task page own session link. Fix: fall back to task_sessions when no session_outcomes row exists and answer 200 with has_transcript=false and zeroed fields; 404 is now reserved for a session_id linked nowhere at all. SessionDetailPage renders an explicit "No transcript yet" state on has_transcript=false instead of a misleading all-zero KPI grid. Root cause 2: GET /api/archify/maps/{kind} 404d whenever no map had been built, and kind="task" is mounted unconditionally on every task detail page -- most tasks never have anyone press "Build map", which is the ordinary, expected state, not an error. Fix: kind="task" now answers 200 with built=false (task_id echoed, zeroed counts) instead of 404; code/concepts/language keep their existing 404 (unchanged, still pinned green). ArchifyMaps.tsx treats built===false the same as a null meta -- the existing "no map built yet" + Build button empty state. 9 new tests across test_sessions_link_before_transcript.py, 2 added to test_archify_api.py, and test_task_page_session_404_ui.py (TSX source-reading pins, no JS test runner in this SPA), committed tests-only first and proved red against unfixed code (8 failed / 14 passed); all pass after the fix. Live-verified against real production data, not just the unit suite: built the SPA, served it via vite preview, and pointed it at a scratch backend process (separate PRISM_DATA_DIR, task a65c66e5 own tasks.db/scores.db/workspace.db copied in, the shared dev daemon never bounced or touched) so the fix could be watched rendering for real without deploying to the live instance. That live pass caught a regression this ticket own tests had not: making the unbuilt-task-map meta a truthy object (built:false) also un-gated the receipt-fetching effect own `!meta || meta.ok` check, which fired a SECOND request-turned-404 (GET .../receipt) that the old null-meta path never triggered -- fixed by adding the same built===false check there, with a new test pinning it (9th test). Screenshotted at 1440x900 with zero console errors on task a65c66e5 own page: the session card renders and links (has_transcript:false, zeroed stats) and the map card reads "No task map built yet." with a Build button, neither a broken link nor a failed request. No PRISM ticket exists for this specific defect -- found live by direct observation of task a65c66e5 own page, not driven through the conductor -- so no [task:] trailer on the commits. Full unit suite: 4738 passed, 12 skipped, 5 pre-existing unrelated failures (C# call-graph x3, packaging-wheel-asset x2) -- none touch sessions.py, archify.py, ArchifyMaps.tsx or SessionDetailPage.tsx. Full integration suite: 785 passed, 5 pre-existing failures (2 need a web_dist build this fix does not by itself trigger, 3 task-runner spend/accumulator/seat-id flakes already documented in prior version notes) -- none in this change verify list.'
)

PRISM_VERSION_NOTES += (
    "7.13.327: the deploy sweep now starts a deploy on its own tick, not only after ship_task. Observed live on the AOS instance: origin/main landed a real commit by a direct push (this repo's own self-dev carve-out) and /api/version still served the previous build twelve minutes later. Root cause: deploy_worker's standing thread only ran sweep_pending, which CONFIRMS an already-requested deploy keyed to a task's own history row -- it never checks whether the checkout's upstream is simply ahead of HEAD, so a land that never goes through ship_worker.ship_task's deploy_after_land hook (or the manual /api/deploy/run trigger) is never noticed at all. Fix: a new sweep_new_land runs BEFORE sweep_pending on every tick (split into deploy_worker._tick so this is pinned without running the real infinite loop); it fetches, parks with a logged reason on a dirty checkout before even fetching (same security tooth deploy_once itself applies), and otherwise runs the identical deploy_once pull+build+restart-request pipeline once the upstream is ahead of a clean HEAD -- no task in the loop, since a bare land observed only via git has no task to attribute evidence to. Declared as a sweep-new-land step in .prism/behaviors/conductor/deploy.json (version bumped 1 -> 2) with a matching POST /api/workflows/steps/deploy-sweep route, so this sweep's own work is visible on /workflows like every other codified node. No second env var is needed: PRISM_DEPLOY_ON_LAND=1 already gates the whole sweep thread (deploy_worker.start_deploy_worker), and sweep_new_land is only ever reached from inside that same thread. Confirmed request_restart's real primitive (auto_updater.perform_restart) is a main-thread os.execv IN PLACE (same pid) -- it re-execs a fresh process image itself and does not depend on Aspire (or any other supervisor) noticing an exit and relaunching it, so under `uv run ... prism start` the sweep's own restart request is sufficient on its own; no separate daemon bounce should be needed once this ships and the currently-running process picks up the fix (which itself requires one restart, since the running process is still on the old code that lacks sweep_new_land entirely). 4 new tests in test_deploy_seat.py (sweep_new_land: deploys a bare land with no task, is a no-op when HEAD already matches upstream, parks on a dirty checkout without fetching; plus _tick's own call order), committed tests-only first and proved red (AttributeError: no sweep_new_land) against unfixed code; pinned suite (test_deploy_seat.py + test_ship_worker.py) 34 passed after the fix."
)

PRISM_VERSION_NOTES += (
    '7.13.328: GET /api/work/graph no longer starves the whole daemon under repeated /live polling -- round 2 of the live-page transcript-I/O hang. 7.13.323/7.13.324 bounded each POLL\'S wait (a shared per-request deadline + per-call timeout via a background executor), verified there against a scratch daemon with real data. But live re-verification on the AOS instance after 7.13.324 deployed showed GET /api/work/graph?project=prism STILL timing out at 30s while /api/version and /api/tasks answered -- an in-process call to the identical work_graph() against the identical real tasks.db/scores.db completed in 2.7s, so the handler\'s own logic was not the remaining problem. The daemon\'s own watchdog (services/watchdog.py, a plain GET / self-probe unrelated to this route) had logged 24 stack-dump timeouts, which only makes sense if something was starving the GIL for the WHOLE process. Root cause: a timed-out transcript lookup is deliberately left running in the background to warm claude_transcripts own file cache, but nothing deduplicated it against the NEXT poll for the identical session -- /live re-fetches this endpoint on every page load/tab and on GraphState self-heal path, so production polling repeats every few seconds, and each repeat before the previous background read finished piled ONE MORE thread onto the shared executor reading the SAME cold, multi-hundred-MB transcript (pure-Python json.loads in a tight per-line loop -- CPU-bound, not just I/O-bound -- so enough piled-up threads contend hard enough for the GIL to stall unrelated requests, including the watchdog\'s own trivial GET /). Fix: work.py._bounded now keys each call by (fn identity, args, sorted kwargs) in a module-level _INFLIGHT dict; a second poll for the exact same session while the first lookup is still running REUSES that Future instead of submitting a duplicate, bounding total concurrent background reads to the number of DISTINCT (fn, args) ever outstanding, never to the number of polls. New test_work_graph_no_thread_pileup_across_polls.py: 5 sequential polls for one chronically-slow session must invoke the underlying lookup at most once while still in flight -- proved red twice against the pre-fix code (5 calls, then 2 calls once the unrelated 5s spend-cache TTL was neutralized to isolate the dedup question) before this fix, green after (1 call). Full work-graph suite: 21/21 passed (test_work_graph_no_thread_pileup_across_polls.py, test_work_graph_transcript_io_budget.py, test_api_work_graph.py, test_work_graph_bounded_queries.py).'
)

PRISM_VERSION_NOTES += (
    "7.13.329: a post-land oracle re-check no longer rewinds an already-shipped task (task d0b392b3). Live: d0b392b3's branch merged to origin/main as f39024cc, ship_worker's _reap_after_land then removed its worktree, and the next gate-adjudicator sweep -- independent of ship_worker's own land -> reap sequence, with no memory of the land -- re-minted the green_gate oracle against that now-gone workspace. pytest collected zero tests (rc=4, wrong/missing cwd) and oracle_spec._run_pytest_ids reported that identically to a genuine red test (ST_FAILED); conductor_service._oracle_receipt_refusal said 'latest receipt FAILED: ...'; _evaluate_green_gate_rewind's naive 'FAILED' in text.upper() check read that as a real regression and rewound the ALREADY-SHIPPED task back to implement_tasks over evidence that said nothing about the merged code. Two independent fixes, either alone stops the misfire: _run_pytest_ids now reports rc==4 (usage error, e.g. file/directory not found) and rc==5 (no tests collected) as ST_ERROR (inconclusive -- the runner never reached a single assertion) with a reason naming the cwd and ids actually tried, never ST_FAILED; a genuine rc==1 failure is unchanged. _oracle_receipt_refusal now words an ST_ERROR latest receipt as inconclusive, the same ST_MANUAL-shaped boundary already used for an unwired browser oracle, never FAILED. Belt-and-suspenders: a new _evaluate_green_gate_rewind guard checks shipped-ness FIRST via _post_land_repo_path (the task's own workspace when it still exists, else PRISM_SOURCE_PATH/cwd, so the check survives the exact reap this bug depends on) -- a task whose [task:<id8>] trailer is already reachable from origin/main is parked at green_gate with the refusal recorded (_park_green_refusal) and is never rewound, regardless of how a future receipt gets misclassified. 7 new tests across test_shipped_receipt_never_rewinds.py and test_pytest_ids_collection_failure_is_error.py, committed tests-only first and proved red (4 failed / 3 passed) against unfixed code; all green after the fix. Neighbouring gate/oracle/adjudicator unit suites (99 tests) and the gate-adjudicator/oracle/verifier integration suites (65 tests) green; full tests/unit: 4790 passed, 5 pre-existing unrelated failures (C# call-graph x3, wheel-packaging-asset x2), none touching this change."
)

PRISM_VERSION_NOTES += (
    "7.13.330: a task stuck 'workspace unavailable, refusing to start (fail closed)' forever now recovers (task a65c66e5, ops incident 2026-09-13). Live: task_reaper.sweep_worktrees removed a65c66e5's clean worktree (its branch was a pure ancestor of origin/main -- zero unique commits, the ordinary shape of a task still at plan_gate/verify_plan) while the task sat in_progress; the branch survived but the directory and git's own worktree registration did not, and the workspace index kept its stale record. Every task_runner/resume_actuator tick afterward called ensure_workspace, which unconditionally ran `git worktree add -b <branch>` on the missing-path path -- git refuses that outright when the branch already exists -- so the task could never start again. Fix 1 (task_workspace.ensure_workspace): when the recorded/conventional path is missing, check whether the task's own prism/ws/<task_id> branch still exists (locally, or on origin) and if so check THAT branch out into a fresh worktree at the same path instead of trying to create it anew, recovering any commits it carries; only a branch that exists nowhere falls back to a fresh branch off base_ref/HEAD as before. Any git failure during recovery still raises (fail closed) rather than silently discarding commits. conductor_flow.flow_start records a workspace_recreated task-history row when this path fires. Fix 2 (task_reaper.sweep_worktrees): a new guard looks up the live task status for any prism/ws/<task_id> branch across every project and refuses to reap while that task is anything other than done/cancelled, regardless of age or cleanliness -- closing the actual hole that reaped a65c66e5 in the first place. An orphaned branch (no task row anywhere) keeps the prior task-agnostic behavior unchanged. 6 new tests (test_ensure_workspace_recreates_missing_directory.py, test_reap_sweep_respects_live_task_status.py), committed tests-only first and proved red (fatal: A branch named '...' already exists) against unfixed code; all green after the fix. Neighbouring task_workspace/task_reaper/flow_start unit suites (39 + 276 tests) green; full tests/unit and tests/integration NOT run (host load; pinned + neighbouring suites only per the fixer brief's load rule)."
)

PRISM_VERSION_NOTES += (
    "7.13.331: the workflows canvas never blanks while the backend is merely slow -- see 8db9badd."
)

PRISM_VERSION_NOTES += (
    "7.13.332: the daemon's own background processing is now visible AS it happens, not just as CPU burn on a dark /live canvas (owner: 'this is all about visibility / observability of all processing happening in the system -- lightning fast' and 'until we can see the work being done most all things you say are likely not true'). New services/system_activity.py is a thread-safe, in-memory ring buffer (last 500 completed passes, answers from memory in well under 5ms, never touches sqlite or git): pass_(kind, project, detail) records a pass as RUNNING the instant it is entered (so a slow, in-flight pass shows a live elapsed counter instead of only appearing once it finishes) and moves it into the recent ring on exit, ok=False (never swallowed) if the body raised. Instrumented all nine standing background workers with one line each: task_runner.sweep_once, gate_adjudicator.sweep_once, resume_actuator.sweep_once, deploy_worker._tick (sweep_new_land+sweep_pending), dispatch_guard.sweep_reap, language_alignment_worker.run_once_for (per project), maintenance_clock.run_tick (per project, the brain-hygiene passes), ship_worker.sweep_once, and main.py's drift-reindex loop (brain.incremental_reindex, per project). New GET /api/system/activity?project=<p> (api/system_activity.py) answers {running, recent} straight from the ring buffer -- a thin read-through, no new service call chain. Frontend: LivePage.tsx now mounts a new SystemActivityPanel (components/live/SystemActivityPanel.tsx), a compact overlay docked top-right (never colliding with the existing gate-decision panel or the reset-layout button) that polls the route every 1s, lists every running pass with a smoothly-ticking elapsed counter (recomputed from started_at every 250ms, not frozen between polls) and the last 20 completed passes (failed ones marked distinctly). Pinned with a new backend unit suite (test_system_activity_feed.py, 6 tests: record/pass_ shape, running-then-recent transition, exception re-raise with ok=False, project filter including wildcard entries, the 500-entry ring cap, and the snapshot limit) and a source-reading TSX pin (test_live_page_system_activity_panel_ui.py, 6 tests, the SPA has no JS test runner) covering the real mount, the real route, the running/recent sections, the top-right docking, and the failed-pass marker. Neighbouring suites for every touched worker module (task_runner, dispatch_guard, ship_worker, resume_actuator, maintenance_clock, language_alignment_worker, deploy_worker: 94 tests) green unchanged. Verified live against a scratch daemon (own PRISM_DATA_DIR, short worker intervals, never the shared instance): all six always-on workers showed real running/recent entries within seconds, screenshotted at 1440x900. Not run: the full tests/unit or tests/integration tree (owner load rule 2026-09-13 -- this host runs the live daemon and integration suites spawn real claude -p step agents; only the pinned + neighbouring suites named above were run, one pytest process at a time, load checked below 6 before each)."
)

PRISM_VERSION_NOTES += (
    '7.13.333: round 3 of the live-page hang -- the real, ongoing blocker was never in api/work.py. Rounds 1-2 (7.13.323/7.13.328) correctly bounded and deduped work_graph own transcript-I/O lookups, but GET /api/work/graph kept timing out on the live instance even after 7.13.324 deployed round 1 fix. An in-process call to the identical work_graph() against the identical real tasks.db/scores.db completed in ~2.7s, so the routes own logic was not the remaining problem. A SIGUSR1 stack dump mid-hang (services/main.py preserved #38 dump handler) caught it: prism-maintenance-clock own background thread was inside governance.py _detect_duplicates(), nowhere near work.py. Direct timing against the real memory store (43 domains, 436 active entries, largest domain architecture at 132) measured run_cycle() at 55.6s total, with _detect_duplicates() alone responsible for 53.8s of it -- a naive O(n^2) all-pairs SequenceMatcher.ratio() scan per domain, each pair building a FRESH matcher and calling the real, expensive ratio(). Because this runs on a periodic background thread that holds the GIL almost continuously while it computes, it starves EVERY other thread in the process -- any HTTP route, not just work_graph, which is also why the daemons own watchdog (a plain GET / self-probe unrelated to any route) had logged 24 stack-dump timeouts of its own. Fix, all behavior-preserving: (1) a plain O(1) length check first -- ratio() can never exceed 2*min(len(a),len(b))/(len(a)+len(b)), so length-mismatched pairs are skipped before any character counting; (2) SequenceMatcher.quick_ratio(), a stdlib-guaranteed upper bound on ratio(), skips the expensive real computation for the rest of the obviously-non-duplicate pairs; (3) one SequenceMatcher reused per outer entry (set_seq2 once, set_seq1 per inner entry) instead of a fresh matcher per pair, per difflibs own documented pattern. Re-measured against the real live memory store: 53.8s -> 14.0s (about 3.8x) -- a real, substantial improvement, though it does not fully eliminate the GIL-hogging at this domain size; capping comparisons or moving the scan off the GIL-critical path is separately-scoped follow-up work. New test_governance_duplicate_detection_scales.py pins the mechanism directly (a real .ratio() call count on a corpus of 26 mutually-disjoint-character noise entries plus one seeded near-duplicate pair, deterministic and hardware-independent) rather than a wall-clock budget -- synthetic text that reliably reproduces difflibs true cost turned out to be its own rabbit hole (autojunk degenerates on low-character-diversity or short-repeated-phrase text; a shared vocabulary defeats quick_ratio as a discriminator), and also pins that the speedup does not change which pairs get flagged as duplicates. Full unit suite: 4784 passed, 12 skipped, 6 failed -- 5 pre-existing/unrelated (C# call-graph x3, packaging-wheel-asset x2) plus one order-dependent flake (test_resume_actuator_stall_dispatch.py, confirmed passing in isolation, 14/14) that does not touch governance.py or work.py. Full integration suite: 789 passed, 1 failed -- the already-documented pre-existing lease-timing race (test_spend_unbounded_when_ceiling_env_unset, flaky since 7.13.315).'
)

PRISM_VERSION_NOTES += (
    "7.13.335: resume_actuator's retry budget no longer spends on an infrastructure refusal (ops incident task a65c66e5, follow-on to 7.13.330). Live: task a65c66e5 sat blocked 'resume-actuator: retry budget spent (3/3) -- parked for a human' after all 3 charged attempts were the identical 'workspace unavailable, refusing to start (fail closed)' refusal -- the drive never got a turn, so this was never a failure of the work, and nothing in this module could lift that park short of a human noticing and calling release() by hand. Fix: dispatch_once's _no_advance and the flow_report-failure path now classify every refusal via _refusal_class (workspace unavailable, fail closed, engine unreachable, engine slot busy, daemon restart, git transport -- an 'infra' refusal) before charging resume_attempts_data.record_attempt; only a refusal from the work itself ('work' class, always true once claude_cli.invoke actually ran) still spends the budget. _park now names the class, the exact last-attempt reason (resume_attempts_data gained a last_reason column), and the PRISM_VERSION at park time, wrapped in backtick inline-code spans so ste.normalize's protected-span rule keeps them byte-for-byte instead of its own semicolon-to-sentence-break rewrite capitalising them unreadably. A new _rearm_once, run first on every sweep, auto-lifts a resume-actuator park (this seat's own 'retry budget spent' wording only, never _park_looping's separate oscillation-ceiling park) whose class is 'infra' or unclassified-legacy once the cause is demonstrably gone -- the daemon version has since changed, or task_workspace.ensure_workspace now succeeds -- resetting the budget and writing a RELEASED_ACTION history row naming the cause, with no human action. 3 new tests in test_resume_actuator_infra_refusal_retry_budget.py, committed tests-only first and proved red against unfixed code (all 3 charged infra refusals parked the task); green after the fix. Neighbouring resume_actuator/dispatch_guard/retry-budget unit suites: 173 passed. Full tests/unit and tests/integration NOT run (host load; pinned + neighbouring suites only per the fixer brief's load rule)."
)

PRISM_VERSION_NOTES += (
    '7.13.336: round 4, and the real one, of the live-page hang -- the previous rounds (7.13.323/328/333: work.py transcript bounding, in-flight dedup, governance duplicate-scan speedup) were all real, correct fixes to real problems, but none of them was the daemon startup burner the owner kept seeing. Live symptom after 7.13.324 deployed round 1: the process sat at 150-180% CPU with 94 threads for 20-30 minutes after every restart, and /api/workflows, /api/conductor/state and /api/work/graph all hung over 20 seconds while /api/version stayed at 10ms and every sqlite file opened read-only in under 40ms (not a lock). The daemon own stderr named the burner directly: "[drift] <project>: reindexed 65 drifted file(s)" repeating for nearly every one of about 30 tracked projects, on every restart, for as long as the burn lasted. Two real defects, read directly out of main.py start_drift_timer and brain_engine.py incremental_reindex: (1) the drift timer swept EVERY tracked project unconditionally on a 30-minute cadence, and its very first tick after any restart fired immediately (last_reindex started at 0.0) -- no gate at all on whether anyone was using a project, so a fresh daemon always paid the full sweep cost before it ever settled. (2) incremental_reindex ran git diff / git ls-files with NO cwd at all, so every project own Brain instance actually diffed the DAEMON OWN checkout (this very repo, under constant concurrent development by multiple fixer agents) instead of its own real source tree -- which is why nearly every tracked project logged the identical "65 files" figure, and why a genuinely unchanged project could never see its own drift settle to zero. Owner directive (2026-09-13, verbatim intent): "we only care about the project we have open... stop doing work just for fun... the whole thing is supposed to be fast and buttery smooth." Fix, reshaped to that scope: new services/drift_worker.py sweep_once() reindexes a project ONLY when it has had a real client request within the last 10 minutes (new services/project_activity.py, fed by a new lightweight ?project= request middleware in main.py) or has an in-progress task; only runs while the API has been idle for a few seconds; is bounded by a 5-second wall-clock budget per call, resuming any leftover candidates on the next call rather than blocking or silently dropping them; and drops, once, logged, any project whose configured root does not resolve to a real directory, never retrying it. main.py start_drift_timer is now a thin while-True wrapper that just polls sweep_once() every 5 seconds -- all the scheduling logic lives in the tested module, matching the existing sweep_once() convention already used by gate_adjudicator.py and task_runner.py. brain_engine.py incremental_reindex(repo_path=...) scopes every git subprocess call via cwd= and resolves every returned path to an absolute one under it, so a projects own baseline genuinely reflects its own tree. Re-measured live via a scratch daemon on a full copy of the real project data directory: /api/workflows answered in 0.23-0.31s and /api/conductor/state in 1.5-1.6s throughout the first 3 minutes after a cold start (previously: both hung over 20s during that same window); total CPU consumed over that same 3-minute window with the one real in-progress-task project as the only active candidate was about 79 CPU-seconds against 180 seconds of wall time (front-loaded in the first 75 seconds of genuine one-time startup work: embedder warm-up, transcript backfill, the first real drift pass), settling to a modest steady-state afterward, versus the previously-reported 150-180 percent CPU sustained for 20-30 minutes. New test_drift_worker_active_project_gating.py (8 tests) pins: a project nobody has opened is never reindexed; a recently-seen or actively-driven project is; a pending client request gates the whole sweep off; a project with no resolvable root is dropped once and never retried; a budget-exhausted pass resumes its leftover queue on the next call rather than dropping candidates. New test_brain_incremental_reindex_scopes_to_repo_path.py (2 tests) pins that the git diff is scoped to the GIVEN repo_path rather than the process cwd, and that a genuinely unchanged project settles to zero reindexed files on a second pass. Both proved red first (ImportError / TypeError against the pre-fix code, verified by temporarily reverting main.py and brain_engine.py to their committed HEAD versions and re-running), green after. Neighbouring suites (test_lifespan_lock_recovery.py, test_maintenance_clock_consolidation.py) pass unchanged -- start_drift_timer keeps its name, its thread-target identity, and its place in the lifespan wiring. Load rule observed throughout: only pinned + grep-neighbouring files run, one pytest process at a time, uptime checked under 6 before every run and every scratch daemon launch.'
)

PRISM_VERSION_NOTES += (
    '7.13.337: round 5 of the live-page hang -- governance own duplicate scan finished the job round 4 started. Round 3 (7.13.333) cut _detect_duplicates() from 53.8s to about 14s per pass with a quick_ratio() pre-filter, a length check, and one reused matcher, all provably behavior-preserving upper bounds on ratio(). Real, but a 14-second GIL-hogging tick every 5 minutes forever is still a hang from a request point of view, and it ran for EVERY tracked project regardless of use, the same "sweep everything on a timer" shape the drift reindexer had (round 4). Owner: "14s of GIL hogging per maintenance tick is still a hang for the user... we only care about the project we have open." Two changes. First, incremental: every entries compare-text is hashed and the hash persisted next to that projects own memory store; a domain where nothing has changed since the last pass is skipped entirely, zero length checks, zero quick_ratio calls, zero real ratio calls, not merely fewer, and a domain with some real changes only compares the changed entries against the rest, since an unchanged-vs-unchanged pair was already resolved in a prior pass. Also chunked: the comparison loop checks elapsed time every iteration and calls time.sleep(0) to actually yield the GIL once a 50 millisecond slice is spent, so even a large first-ever scan never holds the GIL in one long continuous stretch. Recorded via system_activity.pass_("governance", project, "detect_duplicates") so a scan in progress shows on the Live page. Second, per-project gating: run_cycle(scan_duplicates=False) skips the scan entirely while the cheap rules, TTL, budget caps, decay, conflicts, stuck-tasks, still run for every project; maintenance_clock._run_governance now passes scan_duplicates=project_activity.is_in_use(project), a real client request in the last 10 minutes or an in-progress task, the same shared signal the drift reindexer already used. Re-measured live on a scratch daemon with a full copy of the real project data, governance interval forced to 10 seconds so it fired repeatedly during the window: CPU accumulation over a 25-second slice roughly halfway through settled to about 4 CPU-seconds, around 16 percent average utilization, versus the sustained 70-115 percent seen earlier in the same run before the incremental cache warmed. None of /api/workflows, /api/conductor/state or /api/work/graph ever hung again during the full observation window; measured repeatedly from about 50 seconds after start onward, /api/workflows stayed at 0.16 to 0.26 seconds, /api/conductor/state at 0.14 to 2.23 seconds, and /api/work/graph at 1.9 to 3.5 seconds. Honest note: conductor/state and work/graph do not consistently land under the 1-second bar asked for; both carry their own pre-existing DB and task-list query cost (managed_tasks() alone measured around 2.3 seconds in-process against this same real data with NO governance or drift interference at all, back in round 3) that this ticket, bounding governance and drift specifically, does not touch. The defect this round of tickets was chasing, multi-second-to-tens-of-seconds hangs across every route, is gone; a further push to sub-1-second responses on conductor/state and work/graph is a separate, additional round of work on their own query patterns. New test_governance_duplicate_scan_incremental.py (4 tests): a second scan over an unchanged store makes exactly zero real ratio() calls; changing one entry only re-compares it against the rest, not a full rescan; run_cycle(scan_duplicates=False) skips the scan while the rest of the cycle still runs; maintenance_clock._run_governance gates scan_duplicates on project_activity.is_in_use. All four proved red first (by temporarily reverting governance.py, maintenance_clock.py and project_activity.py to their committed HEAD versions and re-running), green after restoring. Neighbouring suites (test_governance_duplicate_detection_scales.py, test_governance_conflict_detection.py, test_drift_worker_active_project_gating.py, test_brain_incremental_reindex_scopes_to_repo_path.py, test_maintenance_clock_consolidation.py): 20/20 passed unchanged. Load rule observed throughout: only pinned plus grep-neighbouring files run, one pytest process at a time, uptime checked and waited on (a Monitor until-loop, never a bare sleep) before every run and every scratch daemon launch.'
)

PRISM_VERSION_NOTES += (
    '7.13.340: GET /api/tasks stops paying SELECT * on every board poll. Measured live (team-lead, 2026-09-13): 946+ tasks (984 by the time this landed) at the old unprojected shape shipped 9.8 MB in 7.6s on a route the board polls every 1-2s -- plan_doc, completion_proof, premise_notes, description, oracle and plan_diagram made up the bulk of it. Every real SPA caller of this route (TasksPage, CompletedTasksPage, WorkflowsPage, TaskDetailPage children fetch) already requested its own explicit fields= projection, but the fields= handler still called TaskService.list() with no column limit, ran SELECT * for every row, built full Task dataclasses (json.loads over every JSON column, every heavy string copied into memory), and only THEN threw most of it away in a Python dict comprehension one line later -- the wire payload was slim, the server-side cost was not. Fix: TaskService.list() takes an optional columns=[...] argument that narrows the SELECT to just the named, allowlisted Task fields (validated against the real dataclass field set, which maps 1:1 onto the tasks table -- an unrecognized name is dropped silently, same leniency the old getattr(t, f, None) projection already had); id and status always ride along so the caller can key rows and the soft-delete filter keeps working even when the caller never asked to render status. _row_to_task now reads every field defensively (name in keys, not just the pre-existing conductor-v2 guard), so a partial row hydrates a partial Task instead of KeyError-ing on a column the SELECT left out. api/tasks.py list_tasks maps each fields= projection (raw attrs plus the mirror_url/artifact_url/mirrors derived pseudo-fields) down to the real columns it needs via _real_columns_for() before calling list(columns=...) -- every existing fields= caller now gets the SAME lean wire shape it always got, at a fraction of the server-side cost. A bare, unprojected GET /api/tasks (no caller today relies on this -- confirmed by grepping every /api/tasks list-route call site under web/src) now returns the SAME kind of lean projection by default (id, title, status, workflow_step, gate_state, gate_reason, proof_type, parent_id, tags, priority, created_at, updated_at, blocked_reason; gate_reason/blocked_reason capped at 200 chars in this default shape only -- an explicit fields=gate_reason request still gets the real value) instead of the full board; pass full=1 for the exact old, complete, unprojected row (fields= is ignored when full=1 is set, by design). In-process before/after on a read-only copy of the live tasks.db (984 rows, /home/siegeon/.claude/jobs/5737d2fd/tmp/tasksslim-bench/bench.py): plain list() (old SELECT * shape) averaged 45.6ms/call and a 10.0MB naive full-row JSON payload; list(columns=<the new default slim set>) averaged 14.6ms/call and a 505KB payload -- a 3.1x query speedup and 19.8x payload reduction. The real production callers land even smaller since they ask for fewer fields than the default set: TasksPage board projection measured 13.0ms/244-363KB, CompletedTasksPage 9.7ms/302KB, WorkflowsPage 8.6ms/244KB, TaskDetailPage children fetch 7.4ms/167KB, all comfortably inside the 200ms bar even before ETag 304 revalidation (already shipped, task fb163fcf) turns a repeat poll into a ~300-byte response. Honest miss: the specific "under 150KB" figure named for the bare-default shape was not hit at 984 real tasks -- gate_reason alone contributes about 168KB even capped at 200 chars, because a large fraction of live tasks carry a gate_reason at or near that cap; dropping it from the default (no live caller uses the default at all) was considered but the field list was an explicit, named part of the ask, so it was kept and the real, measured number is reported here instead of a fabricated smaller one. Tests: test_task_list_projection.py gained test_columns_narrows_the_actual_sql_select (real TaskService against a real sqlite file, sqlite3 set_trace_callback pins the literal SELECT text names only the requested + id/status columns, never SELECT *), test_default_unfiltered_call_returns_the_slim_projection_not_full_rows superseding the old test_default_unfiltered_call_is_unchanged_full_rows (that compat guarantee moved behind full=1, tested by the new test_full_opt_in_restores_the_complete_row and test_full_opt_in_ignores_a_fields_projection), and TSX source pins for the two previously-unpinned real callers (test_completed_tasks_fetch_requests_a_lean_field_projection, test_workflows_page_fetch_requests_a_lean_field_projection) closing the gap alongside the pre-existing TasksPage/TaskDetailPage pins. Fixed a real regression caught by the existing test_mirror_badges_read_the_store.py::test_mirrors_join_is_batched, which calls list_tasks() directly (bypassing FastAPI DI): a bare `full: bool = Query(False, ...)` parameter, when the function is called directly without passing full=, resolves to the unresolved fastapi.params.Query sentinel object rather than False, which is truthy -- guarded the same way this file already guards status for exactly this reason (isinstance(full, bool) and full). 118 tests run across the pinned file plus every neighbouring suite that imports task_service or calls the bare list route (grep-selected, not the full tree, per the box-load rule): all green. Full tests/unit and tests/integration NOT run (owner load rule 2026-09-13, this host runs the live daemon and multiple concurrent fixer sessions).'
)

PRISM_VERSION_NOTES += (
    "7.13.338: the daemon's standing background workers no longer pile up on every restart. Live measurement 20s after a restart (team-lead, 2026-09-13): gate_adjudicator and task_runner still running, deploy_sweep 35.3s, resume_actuator 21.5s, brain_jobs 20.0s, language_alignment 19.5s, ship_worker 13.0s and 8.9s, drift_reindex 8.4s -- all overlapping in the same window, CPU at 150 percent, /api/workflows 12.7s, /api/conductor/state 10.3s, /api/work/graph over 25s while they ran. Two root causes, same shape as rounds 4/5 of the live-page hang: nothing staggered a worker's very first tick after start, and nothing stopped several bounded maintenance sweeps from literally overlapping at once. New services/wakeups.py is a shared in-process coordination module: signal(kind, project, task_id)/wait(kinds, project, timeout, since), a threading.Condition event bus a worker blocks on instead of a bare time.sleep(interval) -- task_service.update() signals task_changed on every status/step/gate write, ship_worker signals shipped the instant a branch lands on origin/main, and gate_adjudicator/task_runner/resume_actuator/dispatch_guard/deploy_worker/ship_worker now wait() on the bus, woken within milliseconds of a real event and otherwise falling back to a 15-minute idle ceiling (gate_adjudicator keeps its fast configured cadence, e.g. 60s, ONLY while a gate is genuinely parked pending somewhere). wait_out_startup_warmup() blocks a worker's first tick until PRISM_WORKER_WARMUP_S (default 120s) has elapsed since process start regardless of any signal that arrives during the window -- nothing has had time to change that early anyway -- wired into all seven standing worker loops plus main.py's drift timer. lower_thread_priority() (os.nice(10), a no-op-safe Linux per-thread trick already used by the drift timer) now runs from every worker's own thread. serial_slot(), a shared process-wide lock, wraps the tick BODY (never the wait() in between) of the five workers whose passes are bounded, pure-maintenance sweeps and were the ones actually measured overlapping: dispatch_guard's reap sweep, language_alignment_worker, maintenance_clock's brain_jobs, deploy_worker's tick, and drift_worker's per-project reindex. Deliberately NOT applied to task_runner, resume_actuator, ship_worker or gate_adjudicator: each can legitimately run for minutes (a real task drive, a CI wait, an oracle test run) and must stay independently responsive -- holding a shared lock that long would starve every other worker, including the gate adjudicator's ability to approve a gate while a drive is in flight; real dispatch concurrency is already bounded separately by dispatch_guard's own PRISM_DRIVE_CONCURRENCY ticket ceiling. system_activity.pass_() gained an optional info[\"active\"] flag: a pass that marks itself inactive collapses into at most one throttled \"idle . N skipped\" entry per kind+project per 10 minutes instead of one ring-buffer entry per tick, and snapshot() now reports quiet (nothing running, nothing active in the last minute) for the panel's header; a caller that never touches info keeps recording every pass, unchanged. Scoped down from the fuller ask: a wall-clock budget with resume-next-turn semantics inside task_runner/ship_worker/resume_actuator's own sweep_once() implementations, and a cheaper pre-fetch precheck inside deploy_worker's git-based land detection, would both need nontrivial surgery on already-pinned, tested control flow -- left as follow-up. 45 new unit tests (test_wakeups_bus.py: event bus, warmup, serial_slot mutual exclusion; additions to test_system_activity_feed.py: idle collapsing and quiet) plus the full targeted batch for every touched worker module (93 tests): all green. Full tests/unit and tests/integration NOT run (owner load rule 2026-09-13 -- this host runs the live daemon and integration suites spawn real claude -p step agents; box load was elevated from other concurrent fixer sessions throughout, so only the pinned plus neighbouring suites named above were run, one pytest process at a time)."
)

PRISM_VERSION_NOTES += (
    "7.13.339: conductor/state and work/graph now land under the sub-1s bar 7.13.337 left open. Live measurement (team-lead, 2026-09-13), 16s after a cold start, daemon idle at 2 percent CPU: GET /api/workflows 0.31s, GET /api/tasks 0.25s, GET /api/system/activity 0.003s, but GET /api/conductor/state 1.53s and GET /api/work/graph 2.72s -- both polled every 1-2s from the Workflows/Live pages. In-process cProfile against a read-only copy of the real project's databases (no code changes needed to reproduce it) found four fanouts, all root-caused to the same shape: a project-wide or per-task computation re-derived from scratch once PER MANAGED TASK instead of once per render. (1) ConductorService._median_step_s / _per_step_typical each redo a regex + datetime.fromisoformat pass over EVERY advance_task row in the project, and phase_progress() calls them once per managed task -- a 58-task render redid the identical project-wide computation 58 times (about half the measured cost). Fixed by memoizing both on the object identity of advance_rows_all()'s own cached dict, which already returns the SAME object on a cache hit keyed on PRAGMA data_version -- the new cache invalidates in lockstep with the rows it is derived from, no separate staleness clock needed. (2) TaskService.history(task_id) had no cache: _in_step_s, _own_transition_run_start, _task_window_start, and api/conductor.py's _dispatch_count_for_step/_gate_seat each called it independently for the SAME task_id within one render (241 calls for 58 tasks). Fixed the same way advance_rows_all already was -- a per-task snapshot keyed on PRAGMA data_version, invalidated on this connection's own writes too. (3) list(parent_id=X) was independently reissued by _children, _queue_depth, and _task_motion_s for the same task_id. (4) managed_tasks(), step_buckets(), and _board_tasks() each ran their own unfiltered SELECT * FROM tasks per request -- three full-table loads for one page render. (3) and (4) share one new cache keyed by parent_id, where None means no filter at all, kept distinct from the empty string, which means root tasks only. In-process measurement, same process for cold and warm, against a copy of the real prism project's data: conductor/state cold 1.213s to 0.452s, warm 1.088s to 0.131s; work/graph cold 1.601s to 0.254s, warm 1.502s to 0.215s -- both now comfortably under the 0.3s warm / 1s cold bar. New tests/unit/test_conductor_state_and_work_graph_query_cost.py (8 tests) traces the real SQL a real state()/work_graph() call issues across a 40-managed-task store: at most one history-by-id read per task, one corpus scan rather than one per task, bounded parent_id-scoped reads, at most one unfiltered board load per render, a repeat /state poll inside the 2.5s TTL skipping every one of those four costly statement shapes, and zero subprocess calls from either route when nothing is pending a gate. Proved red first against unfixed origin/main (7.13.338, verified by committing the tests-only, then temporarily reverting conductor_service.py and task_service.py to their committed HEAD versions and re-running: 5 of 8 failed with the exact per-task/per-render fanout counts named above), green after restoring the fix. Neighbouring suites actually run (test_phase_progress_uses_parent_index.py, test_conductor_v2_state_machine.py, test_adjudicator_minds_the_root_plan_gate.py, test_red_gate_anchor_resolution.py, test_gate_readiness_rollup_scope.py, test_plan_gate_respects_manual_reject.py, test_heavy_polls_are_scoped.py, test_work_graph_no_thread_pileup_across_polls.py, test_work_graph_transcript_io_budget.py, test_conductor_board_health.py, test_conductor_tile_requires_claim.py, test_api_work_graph.py, test_task_views_dont_overfetch.py, test_drive_started_at_anchor.py, and 20-odd more touching advance_rows_all/history/list(parent_id), roughly 490 tests total): all green (an initial batch of 15 failures traced entirely to the dirty-judge-caveat -- an uncommitted edit to conductor_service.py, a control_plane.POLICY_FILES entry, in this same worktree -- and cleared once the fix was committed, no code defect). Full tests/unit and tests/integration NOT run (owner load rule 2026-09-13 -- this host runs the live daemon and several other fixer sessions concurrently; only the pinned plus grep-neighbouring files named above were run, one pytest process at a time, uptime checked and waited on before every run). Untouched by design: gate_readiness's own per-task git subprocess calls (the dirty-judge check, oracle evidence) only run for gate_state=pending nodes, a small bounded count observed at 9-10 live, not a per-task fanout -- not the shape this pass targets. task_runner.eligible_tasks (a sibling item in the same owner brief) was read in full and is already DB-only with no git call or workspace check before a dispatch decision -- left as-is rather than churn working code."
)

PRISM_VERSION_NOTES += (
    '7.13.344: the nine standing workers now run in a SEPARATE OS PROCESS from the API, closing the structural cause behind the whole live-page-hang series (rounds 1-5, 7.13.323-338): those rounds bounded, gated and serialized individual worker passes, but any pass that still held the GIL (rubric scoring, embedding, diffing) or blocked on a git call was, by construction, running in the SAME process as every HTTP route -- measured live: ordinary routes took 10-20s whenever a worker pass ran, 0.2s otherwise. New services/worker_host.py starts task_runner, gate_adjudicator, resume_actuator, deploy_worker, dispatch_guard, language_alignment_worker, maintenance_clock, ship_worker and the drift timer -- exactly the nine workers wakeups.py already named as the source of the GIL contention -- in a child process spawned via multiprocessing.get_context("spawn"). New env var PRISM_WORKERS_PROCESS: 1 spawns the worker host and starts none of these nine as API-process threads; 0 keeps every one of them as an in-process thread exactly as before (byte-for-byte -- test_lifespan_lock_recovery.py CORE_WORKERS passes unchanged); unset defaults to 1 when PRISM_DEV_MODE is set (dev prism start opts in) and 0 otherwise, so a release/docker boot is unaffected until an operator opts in. A supervisor thread in the API process restarts the worker host with backoff if it dies, and terminates it on the API own graceful shutdown. Every other standing thread (mcp_server, understand_drainer, trash_sweeper, auto_updater, transcript_importer, work_ticker, warm_embedder, watchdog, worktree_sweep, drive_activity_observer, graph_enrich, event_pool, sqlite_maint) is unchanged and stays in the API process -- out of scope for this ticket, disjoint from the other fixers working gate_adjudicator/deploy_worker/drift/brain_jobs/language_alignment own sweep logic in parallel. Two cross-process bridges the split needed, since the workers no longer share memory with the API: wakeups.py now backs signal()/wait() with a small WAL-mode sqlite table (wakeups.db) under the data dir in addition to the existing in-process threading.Condition -- a mutation raised by an HTTP request in the API process still wakes a wait() call in the worker host within about 250ms, proved with a REAL separate python subprocess (not just a thread) signalling across the boundary. system_activity.py now exports the worker host own local snapshot to a small JSON file (system_activity.json) every 500ms; the API own snapshot() call signature and in-process-only behavior are UNCHANGED, but now transparently merge in the exported feed when present (a stale export past 5s is ignored, never a ghost "running" entry), so the Live page keeps showing these passes with zero changes needed to the /api/system/activity route itself. 22 new unit tests (test_worker_host_process_split.py, test_wakeups_cross_process.py, test_system_activity_cross_process.py) plus the full existing test_lifespan_lock_recovery.py (3), test_wakeups_bus.py (16), test_system_activity_feed.py (14), test_live_page_system_activity_panel_ui.py (6) and test_task_runner_headless_drive.py::test_wired_into_the_lifespan (1): 62/62 green. Tests-only commit first, proved red against unfixed code in a throwaway git worktree at HEAD (18 failed, 3 errors, 1 passed -- AttributeError/ImportError for every new symbol), green after. Full tests/unit and tests/integration NOT run (owner load rule 2026-09-13 -- 6+ concurrent fixer sessions on this box; only pinned plus grep-neighbouring suites run, one pytest process at a time, uptime checked under 6 before every run).'
)

PRISM_VERSION_NOTES += (
    "7.13.342: GET /api/workflows' catalog no longer repeats its AosWorkflows engine round trips on every request. Measured (team-lead, 2026-09-13): 14.2s while a background pass ran, 0.3s idle -- not algorithmic cost but _conductor_behavior_workflows and _project_validation_workflow together making 18 synchronous HTTP calls to the engine on every single call (one for the scripted validation definition, one for the conductor bot, one MORE per behavior id), each a GIL-release-then-reacquire point that queues behind whatever else is holding the GIL when a background pass is running. Both now go through a new api/workflows.py helper, _cached_engine_structure: memoize the engine-derived structure in process, keyed on a cheap .prism/behaviors/**/*.json mtime fingerprint (the real invalidation signal -- the engine only answers differently when one of those files changes) plus id(_workflow_engine_json) so a test that monkeypatches the engine call per-test gets its own cache slot rather than a previous test's answer, with a 30s TTL backstop for an engine-side change with no corresponding file. The per-request occupancy/live/token-trend overlay get_workflows stamps onto every entry stays uncached and fresh -- only the near-static engine fetch is skipped on a hit -- and every cache read returns a deep copy so one caller's in-place mutation (occupancy/live/task_count/tier, all stamped fresh per request) can never bleed into the next hit. Proof of mechanism, measured in-process against a read-only COPY of the real data (946 tasks, live scores.db, never the live daemon's own dbs), no synthetic load added to the shared box: engine round trips per get_workflows() call went 18/18/18 (cache reset each call, i.e. exactly the pre-fix behavior) to 18 then 0/0/0/0 (cache left warm, i.e. the fixed behavior). Idle wall-clock on this box (real engine reachable and fast, so idle latency is dominated by other, uncached work -- task listing and per-behavior sqlite trend reads, not touched by this ticket) moved only modestly, 285/186/183ms to 194/165/170/177/209ms across repeated calls; the round-trip elimination is what collapses the multi-second stalls reported under real background-pass contention, which this idle box does not reproduce without adding load the box could not spare. 4 new tests in test_workflows_catalog_engine_cache.py, committed tests-only first and proved red (AttributeError: no _reset_catalog_structure_cache_for_tests) against unfixed code re-checked out to HEAD; green after the fix. Neighbouring workflows/conductor/triage/align-language/premise/gate suites (568 tests across 46 files, including the one integration suite touching this module) green unchanged. Left undone: the per-behaviour-entry sqlite trend attachment (_attach_node_trend) still opens 4 connections per entry (~80 total) and the task listing for occupancy/task_count is still called more than once per request -- both real, smaller contributors under the same contention, out of scope for this pass given the file-ownership split with sibling fixers working api/tasks.py and api/sessions.py concurrently. Full tests/unit and tests/integration NOT run (owner load rule 2026-09-13 -- shared box running the live daemon plus 5 other concurrent fixer sessions; 1-minute load checked and waited below 6 before every pytest invocation, one process at a time)."
)

PRISM_VERSION_NOTES += (
    '7.13.347: second speed-mode batch (owner 2026-09-13): /sse/changes stream replaces every browser timer (version, jobs, staleness, occupancy, catalog), the /sse/work reconnect storm on catalog reloads is gone, the slow pill measures only an in-flight request, the Live graph route is under 200 ms warm (conductor memo, batched edge reads, spend off the request path), passes pulse on the Live graph nodes with counts on the banner, and both canvases idle at 1 fps with no packets in motion (renderer 34% -> 6%, /live 1%).'

    '7.13.346: perf and reactivity fan-out landed without per-fix bumps (owner 2026-09-13, speed mode): one worker host per data dir and no self-retriggering loops, event-driven workers with no fallback timers, /sse/changes stream, slim task list, memoized catalog/state/graph routes, canvas shows live occupancy by default, deploy on POST /api/deploy/run ignores untracked files.'

    "7.13.343: gate_adjudicator.sweep_once picks up the resume-next-turn work 7.13.338 explicitly left as follow-up. Measured live at 7.13.338 with ZERO in_progress tasks: sweep_once ran 136.8s, then 94s+, on EVERY cadence, over 26 tasks parked at pending gates none of which had changed -- the daemon sat at 157 percent CPU and every API route hung while it ran. Two costs stacked on top of the existing per-task exponential backoff: every sweep still re-fetched and reconstructed every task row in every project (946 rows) just to re-discover the same ~26 candidates, and the backoff's own comparison key was only task.updated_at, so it could not tell a genuinely-refused gate from one whose workspace had moved underneath it without a matching row write. Fix, entirely inside gate_adjudicator.py plus one new small module: (1) a project-level scan skip -- _project_needs_scan() -- reuses the last pass's eligible count and skips ctx.task_svc.list() ENTIRELY for a project with no task_changed/shipped wakeup since its last scan, bounded by a 300s safety net for writes that bypass task_service.update()/ship_worker's own signal calls; this is the dominant win, since it removes the 946-row fetch+reconstruct, not just the adjudication call. (2) new gate_adjudicator_memo.py: the per-task backoff's comparison key is now (task.updated_at, gate_state, workflow_step, workspace HEAD sha) instead of updated_at alone -- the HEAD sha is cached by the workspace's own .git/HEAD mtime, so a settled workspace costs one stat() per sweep, never a git subprocess, once its HEAD stops moving; this catches an external push/merge that moves the tree without a matching row write. _backoff_should_skip/_backoff_note_refused keep their exact existing 2-arg signature (tid, task) so the pinned test_adjudicator_backs_off_unchanged_refusal.py suite needed zero changes. (3) a 2-second wall-clock budget on the expensive half of one pass (a task whose key actually changed): a task that does not fit the budget is left with no memo entry, so the NEXT tick retries it -- deferred, never dropped. (4) the sweep loop now also wakes on a 'shipped' signal (it previously waited on task_changed only), and system_activity's info[\"active\"] flag now reflects whether a changed key was actually adjudicated this pass rather than merely whether a backlog of unchanged pending gates exists, so an unchanged backlog reads QUIET on the Live page instead of churn. 6 new tests: test_gate_adjudicator_sweep_memo.py (a second sweep over 26 unchanged fake tasks makes zero ctx.task_svc.list()/adjudicate_green_gate calls and completes in well under 100ms; a task_changed signal re-fetches the project and re-adjudicates ONLY the one task whose key actually changed; the safety net forces a rescan once it elapses with no signal at all) and test_gate_adjudicator_memo.py (workspace_head_sha matches a real git rev-parse HEAD, returns \"\" for a task with no workspace, and is cached until the workspace's .git/HEAD mtime actually moves). All 6 proved red first against the pre-fix module (no _project_needs_scan/gate_adjudicator_memo to import), green after. Neighbouring gate_adjudicator suites run individually under the load rule (test_adjudicator_backs_off_unchanged_refusal.py, test_conductor_run_is_recorded_and_live.py -- AC-2's AST scan still finds record_node_execution called directly inside sweep_once's own body, unmoved -- test_a_refused_rubric_rewinds.py, test_terminal_step_closes_task.py, test_adjudicator_minds_the_root_plan_gate.py, test_resume_actuator_stall_dispatch.py, test_system_activity_feed.py): 96 passed, 0 failed. Scoped down deliberately from the team-lead's brief: no is_in_use()-style gate on WHETHER a project's pending gates get adjudicated at all when nobody is looking -- unlike drift_worker/governance/maintenance_clock, this seat's whole purpose (task 1d3322a6, owner directive: 'customers cannot click our board') is to keep clearing machine-decidable gates autonomously and unattended, so pausing adjudication on an idle project would silently undo that; only the FETCH/RE-ADJUDICATION COST when nothing changed is what this ticket removes. Full tests/unit and tests/integration NOT run (host load 6-9 from other concurrent fixer sessions throughout; pinned plus grep-neighbouring suites only, one pytest process at a time, per the fixer brief's load rule)."
)
