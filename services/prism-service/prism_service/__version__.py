...
PRISM_VERSION = "6.0.28"

# Changelog-ish notes (free-form; keep short)
PRISM_VERSION_NOTES = (
    "v6.0.28: Accent palette sweep across all remaining pages. Up to "
    "v6.0.27 the --accent-{tone}-{bg,ring,fg} family covered /memory, "
    "/graph, /understand, /tasks, /brain — but Dashboard, Sessions, "
    "Conductor, Consolidation, Learning, and Retrievals were still on "
    "raw Tailwind primitives (bg-amber-500/15 text-amber-200 etc) or "
    "flat slate sleeves, so the design language broke at every page "
    "boundary. "
    "* /retrievals — mode + rerank chips were flat --midground-base/10; "
    "  now hash-to-tone via toneFromLabel so 'hybrid' / 'bm25' / "
    "  'cross-encoder' each get a distinct stable color. "
    "* /learning — ScorePill ported off raw emerald/amber/rose Tailwind "
    "  onto accent tokens; +N mem chip and 'correlational only' "
    "  warning follow. "
    "* /dashboard — workflow 'running' badge: cyan-400 → accent-emerald; "
    "  Row good/bad indicators ported to accent emerald/amber. "
    "* /consolidation — signal badges (push/fail/bg/mem) keep their "
    "  semantic mapping but ride on accent tokens (bg changes from "
    "  sky to teal so it matches the /understand 'code = teal' "
    "  convention used everywhere). Reflect button states (busy / "
    "  retry / done) all on accent tones. Sparkline trend colors "
    "  mirror ACCENT_HEX hexes so the tiles read in the same color "
    "  language as the in-row badges they're aggregating. "
    "  'not running' worker chip → accent-rose. "
    "* /conductor — persona chip hash-to-tone so each persona reads "
    "  distinctly across the variant table. "
    "* /sessions — skill names hash-to-tone in the usage strip; same "
    "  pattern as memory domains so re-encountering a skill carries "
    "  its color forward. "
    "Settings deliberately untouched here — its semantic colors "
    "(amber=in-progress, emerald=connected, rose=error) already encode "
    "service state correctly; a port to accent tokens is busy-work "
    "without UX gain. Left as a follow-up if needed. "
