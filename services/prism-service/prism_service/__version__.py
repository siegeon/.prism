...
PRISM_VERSION_NOTES = (
    "v7.11.3: THE TASK PAGE STOPS LOADING THE WHOLE BOARD (owner 2026-08-07: "
    "'review all of the calls to the database for the views, we have the "
    "task loading bug again where you are trying to load too much info'). "
    "Second return of the 93d6c6f3 bug; its child dee931c6, which would "
    "have pinned the child-count index, was cancelled. Measured on dev: "
    ...
    "open. Now one indexed query (TaskService.advance_rows_all, 6.6ms). "
    "phase_progress 194ms -> 16ms. (2) Six child lookups loaded all 458 "
    "task rows (every plan_doc/story blob deserialized) to filter parent_id "
    "in Python; now parent_id-scoped onto idx_tasks_parent. activity_for "
    "80ms -> 0.1ms. (3) The turn-token wall-clock fallback re-walked 819 "
    "transcript files / 477MB on EVERY request - no task is link_session'd, "
    "so every page took that path, and it returned zero events for most of "
    "them. Now a merged sorted index behind a 5s TTL, answered by bisect. "
    "The per-file parse was never the problem (18ms warm, already cached); "
    "the repeated filesystem walk was. Also: the Completed and Explore "
    "pages fetched /api/tasks with NO fields= projection - 2.8MB of "
    "plan_doc/story blobs each - to render titles and dates, and to pick "
    "one focus node id. Both now project. Pinned by "
    ...
    "v7.11.2: THE TASK QUEUE IS FIFO (owner 2026-08-07: 'make sure that we "
    "are solving the tasks in the task window in fifo order'). Tasks came "
    "out of TaskService.list ordered priority DESC, created_at ASC, so a "
    "high-priority task filed this afternoon jumped ahead of everything "
    "queued last week - in the board's reading order AND in what the "
    "conductor dispensed next, since next_task and ClaimService.claim_next "
    "both read that same order. Now it is created_at ASC, oldest first, "
    "with priority demoted to a tiebreak between rows sharing a timestamp. "
    "Explicit sorts elsewhere are untouched: the Tasks board's clickable "
    "column headers still re-sort on demand, and Completed still reads by "
    "completed_at. "
