...
def test_catalog_exposes_conductor_and_build_test_validation(tmp_path, monkeypatch):
    ...
    # by task c5650403, then again by task b1971944): "triage",
    # "align_language", "promote_to_law", then "knowledge_health" are real
    # first-class workflows beside conductor/validation, each sourced from
    # models.workflow.WORKFLOWS (or, for knowledge_health, a live metrics
    # read with no steps of its own) the same way conductor's own steps
    # are -- not a new persisted entity, so none violates this test's
    # original "no new entities" spirit; the exact catalog id list just
    # grew by one each time.
