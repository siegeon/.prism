...
def test_default_runner_kills_orphaned_grandchild_on_timeout(tmp_path, monkeypatch):
    ...
    Timing is deliberately generous (not tight): a loaded/noisy-neighbor CI
    runner needs real wall-clock room to fork+exec two fresh interpreters,
    deliver SIGKILL to the whole process group, and have /proc reflect the
    death -- a 2s ceiling / 3s death-poll (the original numbers) passed 9/9
    on a quiet dev box but failed CI-only (task 8b4e7cb6, run 33045863069:
    "grandchild survived the timeout") for exactly that reason. Widening the
    budget changes nothing about what is measured -- SIGKILL-then-poll is
    still the exact mechanism under test.
    """
    from prism_service.services import ship_worker
    ...
    assert elapsed < 20, f"runner took {elapsed:.1f}s; RUN_TIMEOUT_S=5 was not honoured"

    # The pid file is written BEFORE the grandchild's own 60s sleep, so it
    # should already exist by the time the runner (which waited RUN_TIMEOUT_S
    # first) returns -- but give a loaded runner a little more room rather
    # than a bare, racy read.
    file_deadline = time.monotonic() + 5
    while not pids.exists() and time.monotonic() < file_deadline:
        time.sleep(0.1)
