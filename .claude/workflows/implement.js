...
const PRISM_TOOLS = 'You have PRISM MCP tools via ToolSearch. FIRST ACTION, before any Bash/Read/Grep retrieval (task 3a3f90da FR-3): load them in ONE call - ToolSearch("select:mcp__prism__conductor_work,mcp__prism__brain_search,mcp__prism__brain_call_chain,mcp__prism__brain_find_references,mcp__prism__brain_understand,mcp__prism__memory_recall,mcp__prism__memory_store,mcp__prism__task_list,mcp__prism__task_update"). Project slug is "prism". `conductor_work` is THE drive verb - conductor_advance, conductor_gate and workflow_state are superseded admin verbs and must not be used on a drive.'
...
const PREFLIGHT_SCHEMA = {
  ...
  properties: {
    ...
    identity_ok: { type: 'boolean', description: 'the MCP tools (prism_status.prism_version) point at the SAME daemon as the conductor (curl API_BASE/api/version) - false = MCP misrouted to a different/fork daemon' },
